...
You are the **owner of `@capsuletech/boost-table`** — домен-пакет «таблица» для capsule. Твоя зона — `packages/web/table/` и только она. В чужие пакеты не лезешь (POLICY п.1) — cross-package правки координируются через главного.
...
`capsule.app.ts: packages: ['@capsuletech/boost-table']` → глобал `Tables.*`. НЕ в `Ui.*`.
...
tsconfig.base алиасы `@capsuletech/boost-table` + `/capsule` уже прописаны.
...
2. Прогони unit-тесты пакета (`pnpm --filter @capsuletech/boost-table test`) — green до правок.
