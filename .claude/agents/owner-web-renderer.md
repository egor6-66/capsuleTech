...
├── package.json        v0.0.x, peer: solid-js. БЕЗ deps на zod/manifests (это в studio)
└── README.md
```
...
- web-core, web-state, web-router, web-style, web-ui, web-dnd, studio (родственник — authoring side), web-profiler, web-query, shared-zod

`web-renderer` runtime-side того же JSON-tree что редактируется в `studio`. **Любое изменение IEditorNode/ISchema/IInteraction shape** — breaking для обоих. Согласуй с owner-studio.

## Известные грабли

1. **`IEditorNode` дублируется между web-renderer и studio.** Source-of-truth для редактора — `studio/state/types.ts`. Renderer hosts type re-declaration в `types.ts` — поля идентичные. Если расходятся → tree из editor не renderит'ся. Жди ADR-канд: unify в одном месте (вероятно через `shared-renderer-types` пакет).
...
4. **`Renderer` не делает schema validation.** Trust input. Если хочешь validate — используй `studio/manifests` + zod **до** передачи в Renderer.
...
| Хочу… | Куда лезть |
|---|---|
| Новое поле в IEditorNode (например `metadata?`) | `types.ts > IEditorNode` + sync с `studio/state/types.ts` (через согласование) |
...
- **`studio` — родственник** (authoring side того же JSON-tree). Tree shape MUST be идентичен. Любое изменение IEditorNode/ISchema → согласуй с owner-studio.
...
- [ ] **Унифицировать `IEditorNode` shape** с studio (через shared types или один SSOT) — ADR
...
- [owner-studio](./owner-studio.md) — authoring side того же tree
