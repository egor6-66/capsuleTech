...
├── package.json        v0.0.x, peer: solid-js. БЕЗ deps на zod/manifests (это в web-ui-creator)
└── README.md
```
...
- web-core, web-state, web-router, web-style, web-ui, web-dnd, web-ui-creator (родственник — design-time), web-profiler, web-query, shared-zod

`web-renderer` runtime-side того же JSON-tree что редактируется в `web-ui-creator`. **Любое изменение IEditorNode/ISchema/IInteraction shape** — breaking для обоих. Согласуй с owner-web-ui-creator.

## Известные грабли

1. **`IEditorNode` дублируется между web-renderer и web-ui-creator.** Source-of-truth для редактора — `web-ui-creator/state/types.ts`. Renderer hosts type re-declaration в `types.ts` — поля идентичные. Если расходятся → tree из editor не renderит'ся. Жди ADR-канд: unify в одном месте (вероятно через `shared-renderer-types` пакет).
...
4. **`Renderer` не делает schema validation.** Trust input. Если хочешь validate — используй `web-ui-creator/manifests` + zod **до** передачи в Renderer.
...
| Хочу… | Куда лезть |
|---|---|
| Новое поле в IEditorNode (например `metadata?`) | `types.ts > IEditorNode` + sync с `web-ui-creator/state/types.ts` (через согласование) |
...
- **`web-ui-creator` — родственник** (design-time side). Tree shape MUST be идентичен. Любое изменение IEditorNode/ISchema → согласуй с owner-web-ui-creator.
...
- [ ] **Унифицировать `IEditorNode` shape** с web-ui-creator (через shared types или один SSOT) — ADR
...
- [owner-web-ui-creator](./owner-web-ui-creator.md) — design-time side того же tree
