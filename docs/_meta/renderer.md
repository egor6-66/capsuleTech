...
// types.ts
export type {
  ISchema, IEditorNode, IInteraction, RenderMode, Registry,
  IRendererProps, NodeId, IEditOverlayProps, IErrorFallbackProps,
};
```
...
interface IRendererProps {
  ...
  errorFallback?: Component<IErrorFallbackProps>;      // per-node ErrorBoundary fallback
  loadingFallback?: JSX.Element;                       // верхнеуровневый Suspense fallback
  /**
   * ADR 031 — edit-decoration mode. Ортогонален mode.
   * Если задан — для каждой ноды монтируется overlay (position:absolute; inset:0)
   * внутри бокса ноды, без единого замера геометрии. Отсутствует → prod-путь не меняется.
   */
...
- **[[studio|@capsuletech/web-studio]]** — authoring side. `studio/state/operations.ts` производит `IEditorTree`, совместимый с `ISchema.components` 1-в-1. `studio/manifests` описывают компоненты на edit-time (zod-схемы, defaults, drop-валидации), renderer этого ничего не видит. `studio/generators` строит схемы procedurally (то же дерево, что и manual editor).
...
## edit-overlay механизм (ADR 031)

Когда `editOverlay` задан, `RenderNode` переходит в edit-decoration путь:

1. **Void-ноды** (`VOID_NODE_TYPES`: `ui.Input`, `ui.Separator`, `ui.Image` и др.) — не принимают children, overlay нельзя вставить внутрь компонента. Рендерер оборачивает их в:
   ```html
   <span style="display:block; position:relative">
     {компонент}
     <span style="position:absolute; inset:0; pointer-events:none">{overlay}</span>
   </span>
   ```
   `display:block; position:relative` создаёт реальный containing block для absolute overlay. **Не используем `display:contents`**: по CSS-спеке `display:contents` не создаёт own box, поэтому `position:relative` на нём игнорируется браузером — overlay позиционировался бы относительно ближайшего настоящего containing block (родительский контейнер), а не самого инпута.

2. **Не-void ноды** — инжектим `style="position:relative"` в props компонента (merge поверх `node.props.style`). Overlay добавляется в `children` после реальных детей:
   ```tsx
   get children() {
     return <>
       {realChildren}
   ...
   ```
   Это работает только если компонент форвардит `p.children` в DOM. Компоненты которые игнорируют `children` (жёстко закодированный контент) — overlay не получат.

3. `pointer-events:none` на wrapper-span overlay'а — хост решает политику pointer-events внутри своего `editOverlay`-компонента.

## Тесты {#tests}

`packages/web/renderer/src/__tests__/` — 60 тестов (vitest + jsdom + `vite-plugin-solid`):

- `resolve.test.ts` (8) — dot-path lookup, edge-cases (пустой путь, отсутствующие сегменты, intermediate=null).
- `renderer.test.tsx` (53) — basic rendering / fallback / reactivity / interactions (single + multi + unresolved + props) / **thunk-chain Context-regression** / RenderMode / inline-warn dedup / ErrorBoundary / DEV schema validation / runtime props reactivity / **editOverlay (ADR 031)** (9 тестов: без overlay / с overlay / реальный контент / leaf / void / void-обёртка NOT display:contents / ортогональность mode / nodeId передача).

Запуск: `pnpm --filter @capsuletech/web-renderer test`. Любые правки thunk-chain — обязательно через тесты блока «thunk-chain preserves Wrapper Context (CRITICAL)».
