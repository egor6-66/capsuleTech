...
> **Принцип композиции (ADR 041, [[compose-and-inject]]):** большие фичи собираем ИЗ примитивов. Первый шаг по любой фиче — **декомпозиция**: какие переиспользуемые примитивы нужны, что из них УЖЕ есть / шарится с другими фичами. Reuse/extend прежде create — иначе дублируем функционал. Расширяемость — через **инъекцию** (база + точки инъекции, как codegen sub-generators ADR 037), НЕ через флаги. Дистрибуция: тогл = роль-уровень, инжект = app/заказчик (compose-time, только его код).
