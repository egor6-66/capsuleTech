...
| Правка фреймворковых пакетов (`@capsule/core`, `@capsule/state` и т.д.) | Opus |
...
- Когда **компиляторы FSM-схемы** в `@capsule/state/create.ts` обзаведутся `invoke` / `actors` / parallel — соответственно расширяем `controller.md` и `feature.md`.

## Связанное

- [[layers]] — формальное определение HCA-слоёв
- [[golden-rules]] — правила, которые агенты соблюдают
- [[compliance|@capsule/compliance]] — линтер, который ловит нарушения если агент ошибся
