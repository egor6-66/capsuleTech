...
Только frontmatter-поля ниже. `id` = имя файла, `title` = первый H1 тела.

```yaml
type: rule
category: grammar        # phonetics | grammar | speech (группа аккордеона)
order: 20                # порядок ВНУТРИ группы (шаг 10 — удобно вставлять между)
```

Без `category` — возьмётся из папки файла; без `order` — уйдёт в конец (100).

## 📌 Амендмент №2 (2026-07-05) — группировка, ссылки, callouts

1. **Rule**: + `category` + `order` (выше). **Concept**: + `kind:
   approach|pattern|recommendation` + `order`. Твоя стартовая раскладка из
   accordion-предложения принята как есть — проставляй.
2. **Заголовки без нумерации**: «5d. Глаголы и времена» → «Глаголы и времена» —
   go, чисти по всем rule-файлам (порядок теперь несёт `order`).
3. **Wikilinks РАБОТАЮТ**: `[[grammar-verbs-tenses]]` в теле станет живой
   ссылкой в аппе (переход в правило/концепт). Ссылайся по id смело; на
   несуществующий id ссылка просто не сработает (не сломает).
4. **Callouts РАБОТАЮТ**: `> [!info|tip|warning|note] Заголовок` — рендерятся
   красивыми блоками. Используй, не выпиливай.
5. У концепта `relatedRules` теперь виден в аппе чипами «Смотри правила» —
   заполняй осмысленно.
