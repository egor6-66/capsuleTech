...
Shape(
  ...
  (ui, props) => ({ child / columns, … }),  // arg2 CONFIG: row-типизирован, с ui и props (или объект)
);
```

- **arg1** = `schema` (→ row) + `as` (контейнер/шаблон, несёт `__tpl`-маркер). `ui` — для `as: ui.Group` и т.п.
- **arg2** = презентация. Объект ИЛИ `(ui, props) => body`. `ui` — для компонентов внутри (`child.use`, ячейки); `props` — консьюмер-props (для дерайва полей). Row втекает из `schema` arg1.
- **Почему 2 аргумента:** TS не пробрасывает `S` из поля `schema` на соседние поля **того же** литерала (sibling-inference limit). Всё row-зависимое (`child`, `columns`) живёт в **arg2**, отдельно от `schema` → row выводится.
...
// Навигация: as — контейнер, child — шаблон элемента в arg2 (row-типизирован)
...
- `child: { use, props }` (в **arg2**) — **шаблон ребёнка** батча: `use` (компонент элемента), `props: (it) => …` (маппер). НЕ `children` (зарезервированный JSX-проп); НЕ `item`. Внутри `props` `children:` — настоящий проп элемента.
- Простой кейс — без `child`: `as: ui.Button` рисует данные напрямую.
...
1. **Никакого отдельного `R`-дженерика.** В overloads `IShapeWrapper` row-тип инлайнится как `RowOf<S>`, НЕ как `<R extends RowOf<S>>`. Отдельный `R` в контравариантной позиции (`props: (it: R)=>`) TS выводит в `any`. (Было — сломало `child`.)

2. **Row-зависимые коллбэки — только в arg2.** `child.props`/`columns.accessorFn` обязаны быть в arg2 (отдельно от `schema` в arg1). В одном литерале со `schema` → sibling-inference → `it/row: any`. (Было — `item` в arg1 → `any`.)
...
```
`this['row']` валиден **только на top-level property** маркера. Для nested-пропсов (напр. `child`-подобных) оборачивай во внешний generic-helper (`IGroupProps<TRow>`), иначе TS2526. Shape делает `ApplyRow<MarkerOf<typeof as>, RowOf<schema>>` → пропсы шаблона row-типизируются.
...
**(г) Чистые пропсы коллбэков.** Любой prop-коллбэк, типизируемый по row (`columns`/`accessorFn`/`cell`/`child.props`), — через ЧИСТЫЙ интерфейс пакета, не сырой сторонний union (см. §4.4).
...
- batch `child.props` в arg2 (мод B, Navigation) — ✅
