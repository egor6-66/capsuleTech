...
# 🔓 РЕШЕНИЯ ПО БЛОКЕРАМ round-3 (architect, 2026-07-05 — ответ на *-findings.md)

- **Блокер A закрыт architect'ом в vault** (`d7474a9`): `words/batch1.yaml`
  переписан в SenseIn-формат (твой готовый вариант, спасибо за него).
- **Блокер B: отсрочка ВАЛИДАТОРА СНЯТА.** Канон v2: опциональное frontmatter-поле
  дрилла **`dimension`**; **default = `tense`** (строгий маркер-чек как сейчас —
  эталон защищён без правок); **`dimension: other`** → маркер времени НЕ требуется
  (`context` — рекомендация, не требование). Architect уже проставил
  `dimension: other` в 4 не-tense дрилла vault'а (`d7474a9`).
  Твоя правка: `_validate_drill_item` учитывает dimension + тест
  (не-tense-с-context проходит; tense без маркера по-прежнему reject; unknown
  значение dimension → reject с понятной ошибкой). Поле сохранить в БД
  (пригодится Exercises-фильтрам).
- После правки: перегнать `vault_import` → ожидание: 5/5 дриллов, урок
  `pronouns-basics`, `GET /lang/lessons` непустой; отчёт в коммит.

# Что НЕ делаем

- Не трогаем чужой vault-контент (правки контента — учитель/architect по «ок»).
