...
4. **Phase 3 — impl PR (root docs as package), owner-builders + главный.** Создать `@capsuletech/docs` (corner case: package wrapping корневую `docs/`). Удалить `docs/_build/extract.mjs`. Перевести `pnpm docs:build` на новый flow. **Status: done (2026-06-16, branch feat/docs-builder-phase-3).** Phase 3.5 refactor (same branch) расцепил bootstrap cycle: `DocsExtractPlugin` переехал из `lib-builder` в `docs-builder` (canon — вся docs-логика в одном пакете), consumers подключают плагин **явно** через `libConfig({ plugins: [DocsExtractPlugin(...)] })`, lib-builder снова zero-deps leaf.
