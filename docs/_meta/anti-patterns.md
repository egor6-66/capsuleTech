...
## 🚫 Дамп вопросов вместо итерации

**Симптом:** Пользователь обрисовал общую картину фичи (палитра + 2 режима + канвас + варианты + кастом). Агент в ответ выдаёт блок из N «уточняющих вопросов» про каждый элемент картины. Пользователь: «не понял суть ни одного твоего вопроса. зачем тебе канвас если мы делаем палитру?»

**Корень:** Агент смешивает **big-picture brief** (контекст для направления) с **immediate scope** (что делаем СЕЙЧАС). Задаёт вопросы про итерацию 2-3-N до того как итерация 1 началась. Превращает обсуждение в опросник.

**Quick-fix (плохо):**
```
1. Где живёт палитра?
2. Есть ли готовый канвас?
3. Откуда берутся варианты?
4. Группировка через manifest-поле или категории?
5. ... ещё 3 вопроса про следующие итерации
```
→ Пользователь видит дамп, тонет в неактуальных вопросах, теряет фокус, раздражается.

**Proper:**
1. **Повторить картину** как ты понял (1-2 строки) — для confirm-чека.
2. **Зафиксировать scope текущей итерации** — минимум который имеет ценность сам по себе.
3. **Приступить.** Без вопросов про следующие итерации.
4. **Реальные блокеры** для текущей итерации — задавай **точечно**, не накапливай.
5. По ходу — короткие уточнения как возникают. Не дамп.

**Когда вопрос вообще не нужен:**
- Машинальные действия (rename по канону, refactor под структуру, execution docs/POLICY) — делать сразу, без подтверждения.
- Разумное допущение + verify после первого шага — делать допущение, не спрашивать.

**Прецедент:** сессия 2026-06-14, обсуждение Studio Palette итерации 1. Закреплено в `[[feedback_iterations_not_dump]]`.
