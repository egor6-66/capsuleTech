...
description: Index 5 zone canon docs для packages/web/* per ADR 047 D1. Точка входа для контрибьютора / агента / user'а.
status: canon
...
| **boost** | `boost-table`, `boost-map`, `boost-flow`, `boost-charts`, `boost-matrix` | Heavy domain-mirror kit-примитива. Зеркало в `Ui.*`, full power в `Tables.*`/`Maps.*`/etc. | [[web-zone-boost]] |
| **design-time** | `studio` (бывш. `creator` + поглощённый `ui-creator`) | Tooling для создания capsule-приложений. Не в prod-bundle. | [[web-zone-design-time]] |
...
Кратко:
- **kit** зависит ТОЛЬКО на runtime/web-style + vendors. Никогда — на boost/domain/design-time.
...
- **design-time** — на всё кроме apps. Не зависит сам на себя.
...
- [[047-frontend-architecture-zones-cycle-vendor|ADR 047]] — крыша (zones + cycle canon + vendor transparency + studio rename).
