...
- **Editor preview** — `@capsuletech/web-editor` рендерит то же дерево, что юзер редактирует.
...
`@capsuletech/web-editor` хранит дерево в `IEditorTree` (см. `editor/src/state/types.ts`). Поля идентичны `ISchema.components.nodes[id]`, так что renderer консьюмит его без конверсии:
...
- [[editor|@capsuletech/web-editor]] — design-time: manifests (zod), state (операции над деревом), inspector (UI редактирования). Runtime отделён специально — прод-апп консьюмит renderer без overhead'а редактора.
