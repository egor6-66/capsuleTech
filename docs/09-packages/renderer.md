...
> [!info]
> Чистый runtime для рендера UI по JSON-схеме. Принимает дерево узлов + `registry` с компонентами по dot-path'у и эмиттит Solid JSX. Без deps на zod/manifests — authoring концерны живут в [[studio|@capsuletech/web-studio]].
>
...
- **Editor preview** — `@capsuletech/web-studio` рендерит то же дерево, что юзер редактирует.
...
`@capsuletech/web-studio` хранит дерево в `IEditorTree` (см. `studio/src/state/types.ts`). Поля идентичны `ISchema.components.nodes[id]`, так что renderer консьюмит его без конверсии:
...
- [[studio|@capsuletech/web-studio]] — authoring host: manifests (zod), state (операции над деревом), inspector (UI редактирования), generators (procedural UI). Runtime отделён специально — прод-апп консьюмит renderer без overhead'а редактора.
