...
> [!info]
> Design-time зона создания UI: реестр спецификаций компонентов, операции над JSON-деревом, generic-инспектор пропсов и procedural generators. Раньше назывался `@capsuletech/web-editor` — переименован в 0.2.0 чтобы отразить добавление `/generators` (auto-generation поверх manual editor).
...
Корневой импорт `from '@capsuletech/web-ui-creator'` тоже работает — реэкспортит всё.
...
import { manifests } from '@capsuletech/web-ui-creator/manifests';
