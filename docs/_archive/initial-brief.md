...
он ипортирует все компоненты из пакета юай и возвращает, вот как используется import { Render } from '@capsule';
...
это форма находится в entities/viewer/render/fields.tsx, а вот кнопки entities/viewer/render/authButtons.tsx import { Render } from '@capsule';
...
export const AuthButtons = Render(({ Field, Button }) => {
 ...
 теперь что бы с этим работать нужно создать фичу, например features/auth/login.tsx import { Feature } from '@capsule';
...
тут я обрабатываю клик кнопки, и получаю данные из инпутов authForm, это будет храниться в общем стейте с прокси valtio по названию компанента! прокси будет только на компоненты типо Field , тоесть через прокси я смогу ходить по филдам группам и набором групп Field.Set и получать измень значение! при этом момент  return {
  ...
  };я обработал только кнопку логин. значит остальные кнопки не должны появиться в интерфейсе! дальше идет виджет widgets/login.tsx import { Widget } from '@capsule';
