...
Без ADR'а правило **не обязывает**. Любой следующий контрибутор может «по-быстрому» вкатать в `@capsuletech/studio/catalog` свой ad-hoc движок (`web-renderer`-подобный, свой docs-engine, свою CVA-обвязку), и зона расползётся. Studio превратится в свалку «всё для дизайн-тайма».
...
Альтернатива «studio-side overlay» (отдельный JSON в `@capsuletech/studio/catalog`) отвергнута на S2-unification (2026-06-13): manifest объявлен единственным источником правды о компоненте кита. Дополнительный overlay = drift гарантирован.
...
- `@capsuletech/studio/<block>` — каждый subpath это **product-block** (logic-editor, component-builder, inspector-panel, catalog, …).
- Studio **не имеет собственных движков**. Каталог собирается из:
  ...
  - `@capsuletech/studio/docs` (per [[048-docs-as-data|ADR 048]]) — `<DocSection>` для docs-panel.
...
`@capsuletech/studio/catalog` экспортит **независимые модули**:
...
Type union `Catalog.Events` экспортируется из `@capsuletech/studio/catalog` (зеркалит D2 «один subpath с named exports»).
...
export interface IPrimitiveManifestEntry {
   ...
   * Consumed by `@capsuletech/studio/catalog/variants`.
   */
  examples?: IExample[];

  /**
   * Slug'и из docs-registry (ADR 048).
   * Consumed by `@capsuletech/studio/catalog/docs-panel`
   * → `<DocSection slug={...} />`.
   *
...
- **Catalog skeleton (owner-studio)** — единый subpath `@capsuletech/studio/catalog` с 8 named exports (`Palette`, `Preview`, `Inspector`, `ManifestView`, `BundleInfo`, `DocsPanel`, `Contract`, `Variants`) + экспорт `Catalog.Events` union. Эмиссия событий через `useEmit` с namespace'ом `<module>/<event>`. Опциональные manifest-поля не блокируют — модули gracefully degrade при их отсутствии (пустой docs-panel, пустой variants, etc.).
...
- **Studio-side overlay JSON** — расширения manifest'а живут отдельным реестром в `@capsuletech/studio`. Drift гарантирован (правки в одной форме не доходят до другой). Отвергнуто на S2-unification.
