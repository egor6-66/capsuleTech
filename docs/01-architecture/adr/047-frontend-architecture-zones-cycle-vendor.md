...
### Pain 1 — Flat 23-package layout {#pain1}
...
### Pain 2 — Нет канонизированного «зонального» правила зависимостей {#pain2}
...
### Pain 3 — Domain ↔ domain циклы {#pain3}
...
### Pain 4 — Vendor-opaque wrapping {#pain4}
...
### Pain 5 — `web-ui-creator` название врёт; `web-creator` рядом мусорит {#pain5}
...
### D1 — Package zones (5 зон, физическая директория-структура) {#D1}
...
### D2 — Domain isolation canon (no horizontal imports between domain packages) {#D2}
...
### D3 — Vendor transparency canon {#D3}
...
### D4 — Design-time consolidation: `@capsuletech/web-creator` → `@capsuletech/web-studio` {#D4}

Имя `creator` (и тем более `ui-creator`) не отражает scope: это **полноценный design-time** для аппа. Переименовываем в `@capsuletech/web-studio`. Подпути по двум родам — тулзы + редакторы:
...
### D5 — Colocation discipline (heuristic) {#D5}
...
## Что НЕ решает ADR 047 (явно вне scope) {#non-goals}
...
- npm package name `@capsuletech/web-studio` — без изменений.
...
- `PACKAGE_TO_ZONE` map: `@capsuletech/web-studio` мапится в `'studio'` зону.
