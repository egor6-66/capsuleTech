...
> **Финальная форма (уточнена при реализации, см. [[shape-v2-and-table]] §2,4):** arg2 получил `ui`, батч-дескриптор `item` переехал из arg1 в arg2 (sibling-inference). Имя осталось `item` (совпадает с тем, что читают контейнеры web-ui Group/List и web-shell Navigation; промежуточно был `child`, откатан — путался с `children`). Ниже — изначальный набросок; актуальная форма и 4 правила типизации — в reference-доке.

```tsx
Shape(
  (ui) => ({ schema, as }),                  // arg1 — BIND: данные + контейнер (с ui)
  (ui, props) => ({ item / columns, … }),   // arg2 — CONFIG: row-типизирован, с ui+props (или объект)
);
```
