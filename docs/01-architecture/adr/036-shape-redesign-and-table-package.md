...
> **Финальная форма (уточнена при реализации, см. [[shape-v2-and-table]] §2,4):** arg2 получил `ui`, батч-дескриптор переименован `item`→`child` и переехал в arg2 (sibling-inference). Ниже — изначальный набросок; актуальная форма и 4 правила типизации — в reference-доке.

```tsx
Shape(
  (ui) => ({ schema, as }),                  // arg1 — BIND: данные + контейнер (с ui)
  (ui, props) => ({ child / columns, … }),   // arg2 — CONFIG: row-типизирован, с ui+props (или объект)
);
```
