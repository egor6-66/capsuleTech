...
- **`q` script-aware** (`repo.py:filter_senses`): латиница → spelling (word.text), кириллица → gloss (в teacher-корпусе gloss = русский перевод, ADR 064-A). По gloss латиницей осознанно НЕ ищем (q=ase цеплял «pleased»). Эвристика привязана к english-headword корпусу — другие языки headword'ов потребуют ревизии.
