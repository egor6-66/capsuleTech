...
- **Active blockers:** нет (для кода). **Vault-блокер для live lessons-импорта** — см. Quirks: эталонный дрилл rejected пока teacher не проставит `type: rule` в grammar-файлах и не заведёт слово `already` в словарь. Это зона teacher/vocab, не код.
...
- **Lessons-refs — жёсткий resolve, cascade-reject.** Дрилл ссылается на `rule`; если rule-файл rejected (напр. нет `type`), дрилл тоже rejected (ref unresolved). Это by design (правило 4 финалочки) — importer чинит причину не молча. Live-блокер эталонного дрилла (2026-07-04): (1) `grammar-verbs-tenses.md` в vault без `type: rule` → rule не импортится → дрилл reject; (2) слово `already` отсутствует в словаре → `words` reject. Оба — зона teacher/vocab, НЕ код. Импорт корректен, vault не готов.
