...
### 0. 🛡️ КАНОН CAPSULE — модули, не монолит (priority 0)

> 🧱 **БАЗА компонентов — [`docs/01-architecture/component-model.md`](docs/01-architecture/component-model.md). Читать ДО любой UI-работы.** Кратко: любая визуальная композиция — СТРОГО в web-ui пресетом, никогда в пакете/аппе. Карточка = сущность (рисует данные по ключам; вид = пресет, полнота = слоты вкл/выкл, точечно = оверрайд пропсами — НЕ ручной лайаут). Список = пресеты форм отрисовки, элементы = любые компоненты. Свои данные обязаны рисоваться своими компонентами. Регистрация в 2 слоя (manifest + `Ui`-namespace), иначе мимо рендерера/стора studio. **Строишь лайаут в пакете/аппе → используешь не по назначению, СТОП.**

**Эталон** = функционал доведён до конца + покрыт тестами + покрыт документацией. До этого — НЕ канон, временное состояние.

**Архитектура — модули, не монолит:**
- Каждый пакет **самодостаточен**. Один пакет НЕ пускает корни в другой.
- Capsule — фреймворк, **экосистема**. Пакеты — детали этой системы, но детали **независимые**.
- Все пакеты на **100% совместимы** между собой **без адаптеров, без шимов, без костылей**. Совместимость — свойство дизайна, не патчей сверху.

**Решение проблем:**
- Лечим **причину**, не следствие.
- При проблеме НЕ навешиваем костыль поверх канона. Возвращаемся к корню и фиксим там.
- Если канон мешает решить задачу — это сигнал что задача / канон требует пересмотра, **обсуждаем (ADR)**, не обходим.

**Why:** Капсула — фреймворк, основа для реальных проектов. Костыль в framework распространяется на всё что им пользуется (compound debt). Пакет с корнями в соседа = монолит-in-disguise — нельзя релизить независимо, нельзя обновлять независимо, нельзя версионировать независимо. Свойство экосистемы — модульность; её нарушение убивает миссию.

**How to apply (любой агент перед работой):**
1. ПЕРЕД любой правкой: проверь что решение **в корне**, а не на симптом. Если фикс требует hardcoded path / silent fallback / try-catch-swallow / second `@source` — подход неверный, СТОП.
2. ПЕРЕД добавлением depend'а между пакетами: проверь что это не «корень в соседа». Контракт через `interface`/`type`, не через прямой импорт реализации. Pkg A не должен знать **как** Pkg B устроен внутри.
3. ПЕРЕД declare'ом «готово»: проверь три состояния — (1) tests green, (2) docs обновлены (`docs/_meta/<pkg>.md` AI-anchor + user-guide где применимо), (3) реальный e2e сценарий проходит (`capsule build && capsule preview` для UI; smoke fixture для CLI/build).
4. Если задача упирается в gap пакета: gap → фикс в **пакете-владельце** (owner-X), НЕ workaround в consumer'е.

### 0.1. 📦 GIT SCOPE PR = ФОРМА ИЗМЕНЕНИЯ (не догма)

Scope PR'а определяется **логической формой изменения**:

| Изменение | Scope PR'а |
|---|---|
| Локально в одном пакете (фича/рефакторинг/тесты внутри owner-зоны) | **Per-package.** Owner-X делает PR, мержит после CI. Чистая releasability. |
| Cross-package баг/фича (одна логическая цепочка через несколько зон) | **Per-feature/bug.** Один PR со скоупом фичи, координирует architect, owner'ы работают по своим файлам в этом PR. |
| Mixed WIP «куча всего» | Аномалия. Лечится одноразовым cleanup-PR. Не норма. |

**Pre-эталон vs post-эталон.** Канон §0 «пакеты на 100% совместимы без адаптеров»:
- Pre-эталон — cross-package PR частые = **симптом неготовности границ**, не нарушение.
- Post-эталон — cross-package PR редкие = либо contract change (легитимно), либо canon-violation (фиксим причину).

**Что НЕ меняется:** scope ≠ авторство. Owner коммитит **всё** что в его scope попало, независимо кто строки писал. Cross-package PR не фрагментируется post-factum — один PR = одна логическая единица.

### 0.2. 🚦 GIT HARD-GATE (harness-хук, не промпт)

PreToolUse-хук `.claude/hooks/git-gate.mjs` физически режет write-операции git/gh на shared working tree:
...
SessionStart-хук `.claude/hooks/dev-diagnostics-session-start.mjs` детектит активный log (mtime < 60 сек) и подсказывает в context'е команду для `Monitor` — каждая новая строка лога станет notification'ом. Без активного dev — silent skip, регрессий нет.

Принцип: **agent судит по реальности компиляции, не по памяти и тексту в файле**. Если ты думаешь «может, типы сломаны?» — не гадай, запусти dev и посмотри в стрим.
...
- **Sidestep (2026-05-31, актуальный):** `infinite: { mode: 'plain' }` — non-virtual бэкенд `createInfiniteScroll` (headless-хук в `@capsuletech/web-ui/lib`, рядом `createPagination`) рендерит все подгруженные строки без виртуалайзера → quirk'а нет. ewc-таблица переведена на plain (PR #205). Используй plain пока virtual не починен.
