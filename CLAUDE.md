...
`@capsule/source` — экспериментальный фреймворк поверх Solid.js + XState + TanStack Solid Router. Архитектура называется **HCA (Hyper-Controlled Architecture)**, философия: *"UI is a Shadow"* — интерфейс это немая проекция логики; вся власть в Controller/Feature, общение с UI идёт через Proxy и meta-теги.
...
- **Build:** Vite 6 + кастомные плагины (`@capsule/shared-vite`)
...
nx g @nx/js:library --name=@capsule/X --directory=packages/X --importPath=@capsule/X --publishable --buildable
...
> [!important]
> Vite привязан к **директории приложения** (`apps/<app>/`) — она же `workspaceRoot` для `getWorkspaceRoot()` в `@capsule/file-manager`. Запуск из корня репо ломает резолв `capsule.config.ts` и алиасы. Все CLI-команды дёргают через `cd apps/<app>` или скрипты приложения.

Запуск sandbox через CLI на самом деле дёргает `createDevServer` из `@capsule/core/builder`, который читает `apps/sandbox/capsule.config.ts` и кормит конфиг в `@capsule/shared-vite`.
...
`tsconfig.base.json` определяет `@capsule/*` пути для IDE. Реальные алиасы для рантайма — в `packages/core/src/builder/config.ts → resolve.alias` (для dev/build). При добавлении нового пакета обновляй **обе точки**, иначе IDE будет видеть, а Vite — нет.
...
✅ Правила **enforced линтером** через `@capsule/compliance` (Vite-плагин `CompliancePlugin`, режим `warn`). При нарушении — warning в логе dev-сервера с file:line:column + hint что делать. См. ADR 004.
...
- `packages/ui/vite.config.ts` импортирует `defineConfig` из `@capsule/shared-vite`, которого там нет (см. `pnpm nx build core` на старте сессии). Сборку core это не ломает, но `nx run-many -t build` для `ui/router/style` упадёт. Будет в отдельном проходе.
...
- ✅ Compliance-линтер — `@capsule/compliance` + Vite-плагин ловит upward / horizontal / disallowed-import / fetch-в-Controller (ADR 004, режим `warn`).
...
| `ui-component` | Haiku | новый компонент в `@capsule/ui` |
...
**Что НЕ делегируется:** архитектурные решения, ADR, кросс-слойные рефакторинги, правка фреймворковых пакетов (`@capsule/*`), code-review результата субагента, чистая косметика. Это всё делает главный (я).
