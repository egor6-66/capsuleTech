          ...
          rows={[
            {
              ...
              cells: [
                {
                  id: 'main-cell',
                  resizable: false, // opt-out: ensure non-corvu path within row
...
describe('packing resize handle independent of row.resizable', () => {
  it('resize handles appear in packing zone when row.resizable=false (resize=true)', () => {
    // This is the key invariant: renderPackingRow gates the resize handle on
    // resize mode only, NOT on row.resizable. A non-resizable zone (no corvu
    // splitter) still shows cell-level packing handles when resize is enabled.
    cleanup = render(
      () => (
        <Matrix
          dnd="insert"
  ...
  it('resize handles absent when resize=false regardless of row.resizable', () => {
    cleanup = render(
      () => (
        <Matrix
          resize={false}
