/**
 * Markdown — рендер markdown-тел концептов/правил (там таблицы + callout'ы +
 * wikilinks!). Переиспользует `renderMarkdown` из `@capsuletech/web-docs` (та
 * же механика, что `DocSection` инжектит README в studio Info) — не тянем
 * новый markdown-dep. Контент курируемый (lang-vault, ADR 069), поэтому
 * `innerHTML` безопасен — тот же контракт, что `web-docs`.
 *
 * Атом `shared/markdown/` — готовый рендер, используется повсеместно
 * (View/Concept/Rule сейчас, любой markdown-контент дальше), не собственность
 * `lessons`. Промоутнут в `Learn.Markdown` (был internal). Направление
 * строгое: `modules/ → shared/`, НИКОГДА обратно.
 *
