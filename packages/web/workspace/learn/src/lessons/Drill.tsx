/**
 * Drill — internal-интерактив дрилла (building-block `View`, отдельно НЕ
 * регистрируется). Каждый item: `promptRu` (+`context`) → Input + «Проверить»
 * → вердикт (✅ / хинт near_miss / «мимо» wrong); «Показать ответ» = reveal.
 * Ключ ответа на бэке — фронт только показывает то, что вернул чекер.
 *
 * Слова дрилла (`words_resolved`) — 🔊 через `onSpeak`-колбэк (проброшен из
 * `View`, канал `onSpeak { audioUrl }`; плеер/движок — app-concern).
 *
...
export const Drill = (props: IDrillProps) => (
  <Layout.Flex orientation="vertical" gapY={2}>
    ...
    <For each={props.drill.items}>
      {(item) => {
        const verdict = () => lessonsStore.verdict(props.drill.id, item.index);
        const submit = (reveal: boolean) =>
          void lessonsStore.check(props.apiBase, props.drill.id, item.index, reveal);
        ...
        return (
          <Layout.Flex orientation="vertical" gapY={1}>
            ...
            <Layout.Flex orientation="horizontal" gapX={1} align="center">
              <Input
                value={lessonsStore.answer(props.drill.id, item.index)}
                placeholder="Ваш ответ…"
                onInput={(e) =>
                  lessonsStore.setAnswer(props.drill.id, item.index, e.currentTarget.value)
                }
              />
              <Button
                size="sm"
                disabled={lessonsStore.checking(props.drill.id, item.index)}
