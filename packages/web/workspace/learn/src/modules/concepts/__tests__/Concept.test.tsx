/**
 * Learn.Concept tests — URL-driven по `id` через `Ui.Article`: title + принцип
 * (lead) + тело в `Prose` (ведущий H1 срезан) + примеры + чипы relatedRules
 * («Смотри правила») → emit `onRuleSelect`; wikilink → `onConceptSelect`.
 * Fallback до выбора → `Placeholders.Empty`. `useEmitOptional`/`renderMarkdown`
 * мокнуты; `emitRefNav` (core) читает реальные `conceptsStore`/`rulesStore`.
 */
