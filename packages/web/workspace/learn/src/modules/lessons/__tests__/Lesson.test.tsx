/**
 * Learn.Lesson tests — маршрут по порядку (intro → concepts → rules → drills),
 * дрилл-флоу (correct / near_miss с хинтом / wrong / reveal) и emit `onSpeak`
 * со слова дрилла. Дрилл-интерактив идёт через `drillsStore` (через UI).
 *
 * `useEmitOptional` мокнут (прецедент `Words`); `renderMarkdown` мокнут, чтобы
 * не тянуть тяжёлый граф `@capsuletech/web-docs` в юнит.
 */
...
import type { ILessonDetail } from '../types';
