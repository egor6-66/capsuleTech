/**
 * Learn.Lessons tests — lazy-load списка на mount, рендер по уроку, клик →
 * `lessonsStore.open` (fetch урока) + emit `onLessonSelect { id }`.
 *
 * `useEmitOptional` мокнут (прецедент `Learn.Words`).
 */
...
describe('Learn.Lessons', () => {
  it('lazy-loads the list on mount and renders one card per lesson', async () => {
    cleanup = render(() => <Lessons />, container);
    await flush();
