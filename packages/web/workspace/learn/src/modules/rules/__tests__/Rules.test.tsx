/**
 * Learn.Rules tests — аккордеон по `category`: порядок групп + ru-подписи +
 * подзаголовки; СВЁРНУТ по умолчанию, кроме группы активного `id` (через
 * `aria-expanded`); внутри — по `sortOrder`; клик по карточке → emit
 * `onRuleSelect { id }`. `useEmitOptional` мокнут.
 */
