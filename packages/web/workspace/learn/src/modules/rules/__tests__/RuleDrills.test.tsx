/**
 * Learn.RuleDrills tests — практика правила (URL-driven по `id`): секция
 * «Практика» с дриллами из кэша правила; дрилл-флоу correct (общий чекер
 * `drillsStore`) + emit `onSpeak`. Ключ: смонтированный РЯДОМ с `Rule` на тот же
 * id — ОДИН fetch правила (общий кэш/дедуп `rulesStore`). Fallback →
 * `Placeholders.Empty`. `useEmitOptional`/`renderMarkdown` мокнуты.
 */
/* @vitest-environment jsdom */
import { render } from 'solid-js/web';
import { afterEach, beforeEach, describe, expect, it, vi } from 'vitest';
import type { ICheckResult } from '../../drills/types';
