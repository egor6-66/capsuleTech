/**
 * Learn.Rule tests — URL-driven по `id` через `Ui.Article`: title + тело в
 * `Prose` со СРЕЗАННЫМ ведущим H1; БЕЗ дриллов (уехали в `RuleDrills`); wikilink
 * в теле → emit `onRuleSelect`/`onConceptSelect` (по загруженным спискам),
 * неизвестный ref → `console.warn` + no-op. Fallback → `Placeholders.Empty`.
 *
 * `renderMarkdown` мокнут (echo — тело = сырой HTML под настоящий
 * `<a class="wikilink">`); `useEmitOptional` мокнут; `emitRefNav` (core) читает
 * реальные `rulesStore`/`conceptsStore`.
 */
