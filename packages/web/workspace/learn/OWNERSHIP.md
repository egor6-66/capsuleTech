...
status: skeleton
priority: P2
last-updated: 2026-07-06 (nav consolidation → shared/segments + modules/navigation + modules/welcome, Learn.Nav.*/Welcome.*)
---
...
src/
  ...
  shared/                   ← АТОМЫ (переиспользуемые блоки, юзают многие модули)
    ...
    segments/               ← единый источник nav-сегментов (main/library/lessons) — читают navigation/ + welcome/
  modules/                  ← фиче-композиты (папка-на-модуль-СУЩНОСТЬ), импортят из shared/ + core/
    ...
    navigation/             ← MainNav/LibraryNav/LessonsNav (nav-блоки) → Learn.Nav.{Main,Library,Lessons}
    welcome/                ← Welcome/LessonsWelcome/LibraryWelcome (лаунчеры) → Learn.Welcome.{Root,Lessons,Library}
    exercise/ guides/ library/ progress/ sentence-builder/
...
- **`@capsuletech/web-shell`** (workspace, dep) — `SegmentNav`/`Launcher`/`Header.Navigation` из `/ui`. Nav/Welcome-блоки зоны (`modules/navigation`/`modules/welcome`) = композиция этих connected-блоков + сегменты зоны; визуал/routing/emit sub-nav'ов и лаунчеров — внутри web-shell.
- **`@capsuletech/web-router`** (workspace, dep) — `useActiveSegment` для `Learn.Nav.Main` (главный header-nav). Пилот дедупа (`pilot-segment-nav-4-learn`) убрал web-router из deps, т.к. sub-nav'ы деривят active внутри web-shell; **nav-консолидация вернула его** (brief `web-learn-nav-consolidation`): `MainNav` — пакетный блок поверх `Shell.Header.Navigation`, который сам деривит active из URL (web-shell не трогаем per brief), поэтому learn снова напрямую зовёт `useActiveSegment`. Sub-nav'ы (`SegmentNav`) по-прежнему деривят внутри web-shell.
...
| `./welcome` | **welcome-лаунчеры зоны** (`modules/welcome`): `Root`/`Lessons`/`Library` (регистрируются `Learn.Welcome.{Root,Lessons,Library}`) — тонкие data-биндинги над `Shell.Launcher` + сегменты из `shared/segments`. Реэкспорт `MAIN_SEGMENTS`/`IMainSegment`/`MainSegmentId` (был `LEARN_SEGMENTS`) для backward-compat типизации. Nav-консолидация вернула UI в папку (пилот дедупа держал только данные) |
| `./controllers` | гнездо `Controllers.Learn` (ADR 032) — пока пусто (`export {}`) |
| `./capsule` | `capsule.tsx` — `defineCapsuleModule({ name: 'Learn', components })` (ADR 033), **тонкий** (только импорты + реестр, ноль inline-JSX после nav-консолидации). Вложенные неймспейсы: `Nav.{Main,Library,Lessons}` (nav-блоки из `modules/navigation`), `Welcome.{Root,Library,Lessons}` (лаунчеры из `modules/welcome`), `Library.{Info}`. Плоские: атомы `Words`/`Search`/`Markdown` (из `shared/`), lessons-домен `Lesson`/`Lessons`/`Concept`/`Concepts`/`Rule`/`Rules`/`RuleDrills`, `Provider`/`Exercise`/`Progress`/`Tour`/`SentenceBuilder`/`Collections` |
...
- **Nav/Welcome — пакетные блоки в своих папках, композиция shell-блоков (nav-консолидация, brief `web-learn-nav-consolidation`).** learn = app-host: ноль своего nav/welcome-UI/классов, только данные (`shared/segments`) + композиция готовых web-shell блоков. `modules/navigation`: `MainNav`(`Learn.Nav.Main`) поверх `Shell.Header.Navigation`, `LibraryNav`/`LessonsNav`(`Learn.Nav.{Library,Lessons}`) = `Shell.SegmentNav`. `modules/welcome`: `Welcome`/`LessonsWelcome`/`LibraryWelcome`(`Learn.Welcome.{Root,Lessons,Library}`) = `Shell.Launcher`. Раньше (пилот дедупа) биндинги жили inline в `capsule.tsx`, main-nav — в АППЕ (`shapes/shellNavigation`); консолидация подняла nav в пакет и раскидала по папкам, `capsule.tsx` стал тонким.
  - **Вложенный неймспейс `Learn.Nav.*`/`Learn.Welcome.*`** (решение user: не плодить `MainNav`/`LibraryNav`/… имена). Безопасно: у nav/welcome-блоков НЕТ своих `__events` (контракт события — generic `onSegmentNavigate { nav, segment }` из `Shell.SegmentNav.Events`), а вложенность влияет только на рендер (прецедент `Learn.Library.Info`). Регистрация = spread всего `components`-объекта в глобал `Learn` (`registry/packages.ts`), вложенные ключи резолвятся как обычный property-access.
  - **Событие единое `onSegmentNavigate { nav, segment }`** — app-Feature различает источник по `payload.nav` (`root`/`lessons`/`library`). `MainNav` эмитит `nav: 'root'`.
  - **`MainNav` реактивность active-state:** `Shell.Header.Navigation` (batch над `ui.Group`) рендерит items через `<For>` — props-маппер вызывается ОДИН раз на item, НЕ реактивен по `active()`. Поэтому подсветку (`variant`/`aria-current`) держим внутри item-компонента `NavButton` (его JSX реактивно читает `active()` из `useActiveSegment`), а не в статичном `item.props`-мапе. `data={[...MAIN_SEGMENTS]}` — спред снимает `readonly` (Group.data мутабельный).
- **`guides` в едином наборе `MAIN_SEGMENTS`** — до консолидации main-nav жил в двух источниках: app-shape (`Guides`→`/guides`) и `LEARN_SEGMENTS` (без guides = дрейф). Единый источник = ВСЕ top-level секции с модулем+роутом → `guides` добавлен. Следствие: карточка Guides появляется И в header-nav, И в welcome-лаунчере (`Learn.Welcome.Root` кормится тем же набором). Это цена единого источника — обсуждаемо, если welcome не должен показывать guides, набор надо расщепить (nav-набор ⊇ welcome-набор).
...
- [x] Nav-консолидация — `shared/segments` (единый источник main/library/lessons, снят дубль app↔pkg + дрейф guides), `modules/navigation` (`MainNav` из app-Shape поверх `Shell.Header.Navigation` + `Library`/`Lessons` из inline), `modules/welcome` (лаунчеры из inline), вложенные `Learn.Nav.*`/`Learn.Welcome.*`, тонкий `capsule.tsx`, +`@capsuletech/web-router` dep (brief `web-learn-nav-consolidation`) — 2026-07-06. App-сторона (снос `shapes/shellNavigation`, рефы ключей) = companion brief `apps-learn-nav-from-package` (apps-learn).
...
| Unit | `modules/navigation/__tests__/MainNav.test.tsx` | `Learn.Nav.Main`: кнопка на каждый `MAIN_SEGMENTS` (incl. `guides`), id'шники в `useActiveSegment`, active→`aria-current=page`, клик → emit `onSegmentNavigate { nav: 'root', segment }` (`source: 'Learn.Nav.Main'`) |
