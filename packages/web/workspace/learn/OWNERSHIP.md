...
| `./library` | **Реальный** library-браузер (перенесён из `apps/learn`, канон «пакет владеет стором» — см. Quirks): `Search` / `Words` / `Info` (регистрируются вложенно `Learn.Library.Search`/`.Words`/`.Info`) + `libraryStore` singleton (`load`/`select`/`selected`) + `fetchSenses` (`api.ts`) + `ISense` тип. Плюс прежние плейсхолдеры: `Collections` / `VocabList` / `BookmarkButton`. **`LibraryNav` и `LibraryWelcome` больше НЕ блоки зоны** — композиция `Shell.SegmentNav`/`Shell.Launcher` + `LIBRARY_SEGMENTS` в `capsule.tsx` (пилот дедупа, см. Quirks). `LIBRARY_SEGMENTS` internal (не реэкспортится); `LibrarySegmentId`/`ILibrarySegment` реэкспортятся из `./library` (типизация сегментов) |
...
- **⚠️ Остаточные сырые классы (surface, НЕ в scope пилота).** Бриф заявлял «ноль сырых классов в learn после дедупа» — но снесены были только Nav×2/Welcome×3. `lessons/Concepts.tsx` и `lessons/Rules.tsx` (аккордеон-дерево, ИА iter 2) до сих пор несут сырой Tailwind на строках-темах (`flex w-full cursor-pointer …`). Это отдельная работа (промоут tree-item в web-ui примитив, зона owner-web-ui), не этого брифа. Зафлажено architect'у.
