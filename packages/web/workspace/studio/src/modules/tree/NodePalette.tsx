...
import { manifestsForNode } from '../../shared/manifests';
...
export const NodePalette = (props: INodePaletteProps) => {
  ...
  return (
    // Индент по depth — динамический padding-left (kit-пропа нет), inline-style.
    <div style={{ 'padding-left': indent() }}>
      {/* RESIDUAL (kit-gap): `border-0` (Item) + `px-0 py-1` (Trigger) — flush-
          de-chrome kit Accordion под inline-встройку в дерево. Нужен flush/
          unpadded вариант Accordion (owner-web-ui, обсудить). `divide-y-0` grep-safe. */}
      <Accordion multiple class="divide-y-0">
        <Accordion.Item value="add" class="border-0">
          <Accordion.Trigger class="px-0 py-1" data-testid={`node-add-${props.nodeType}`}>
            <Typography as="span" size="xs" weight="normal" tone="muted">
              ＋ добавить компонент
            </Typography>
          </Accordion.Trigger>
          <Accordion.Content>
