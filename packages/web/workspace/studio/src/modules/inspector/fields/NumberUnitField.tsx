...
export const NumberUnitField = (props: IProps) => {
  ...
  return (
    <FieldShell label={props.field.label} hint={props.field.hint}>
      {/* RESIDUAL (kit-gap): kit.Input/kit.Select не имеют grow/shrink/width-
          пропов — `flex-1 min-w-0` (число тянется) и `w-20 shrink-0` (единица
          фикс) остаются классами. Нужны sizing-пропы у Input/Select (obсудить). */}
      <Flex gap={1}>
