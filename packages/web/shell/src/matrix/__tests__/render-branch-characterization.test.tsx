...
describe('renderContent — Branch 2: direction=horizontal', () => {
  ...
  it('B2b: non-resizable horizontal zones → plain flex-row (no corvu)', () => {
    ...
    // Cells must explicitly opt out of per-cell resize (resizable:false) so
    // rowHasResizable() returns false and the non-corvu path is taken.
    cleanup = render(
      () => (
        <Matrix
          direction="horizontal"
    ...
    // Non-resizable cells → no corvu panels
    expect(container.querySelector('[data-corvu-resizable-panel]')).toBeNull();
  });
});
