/**
 * Тесты EditorController handlers.
 *
...
const makeCtx = (overrides: Partial<IEditorCtx> = {}): IEditorCtx => ({
 ...
 */
const makeApi = (ctx: IEditorCtx, payloadValue?: unknown) => {
...
// Схема определена внутри EditorController.tsx — извлекаем через re-export
...
describe('EditorController — onSelect toggle logic', () => {
  it('select нового узла', () => {
    ...
    const current = (api.context as IEditorCtx).selectedId;
    api.store.update({
...
describe('EditorController — onDrop мутирует tree', () => {
...
describe('EditorController — onMark', () => {
  it('установить метку для ноды', () => {
    ...
    const next = { ...(api.context as IEditorCtx).marks };
...
describe('EditorController — drag-cycle', () => {
