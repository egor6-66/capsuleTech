...
type _B3_IsRowActiveOk = Expect<Equal<Parameters<NonNullable<_B3BaseProps['isRowActive']>>[0], Incident>>;
type _B3_ItemPayloadOk = Expect<Equal<Parameters<NonNullable<_B3BaseProps['itemPayload']>>[0], Incident>>;
type _B3_GetRowIdNotAny = Expect<Equal<IsAny<Parameters<NonNullable<_B3BaseProps['getRowId']>>[0]>, false>>;
...
type _ER_GetRowIdNotAny = Expect<Equal<IsAny<Parameters<NonNullable<_ERProps['getRowId']>>[0]>, false>>;

// ER ПРОХОДИТ: негатив — nonExistent ошибка
const _erNegative = ShapeER<Incident>((zArg) => ({
  schema: zArg.array(IncidentSchema),
  columns: [{ accessorFn: (row) =>
    // @ts-expect-error — nonExistent не существует в Incident
    row.nonExistent,
  }],
}));
...
type ApplyRowFrom<A extends { __tpl?: unknown }, R> =
  ApplyRow2<
    MarkerOf<A> extends { row: unknown; props: unknown } ? MarkerOf<A> : { row: unknown; props: Record<string, never> },
    R
  >;
...
type _S8_Data_ExtendsIncidentArr = Expect<Equal<_S8_DataActual extends (Incident[] | undefined) ? true : false, true>>;
type _S8_Data_IncidentExtends = Expect<Equal<(Incident[] | undefined) extends _S8_DataActual ? true : false, true>>;
...
declare function Shape3<S extends z.ZodType, A extends { __tpl?: unknown }>(
  cfg: { schema: S; as: A },
): (
  factory: (ui: any) => Partial<ApplyRowFrom<A, RowOf<S>>> & { defaults?: z.infer<S> },
) => (props: ApplyRowFrom<A, RowOf<S>> & { data?: ShapeData<S> }) => unknown;
...
type _S3_Data_ExtendsIncidentArr = Expect<Equal<_S3_DataActual extends (Incident[] | undefined) ? true : false, true>>;
type _S3_Data_IncidentExtends = Expect<Equal<(Incident[] | undefined) extends _S3_DataActual ? true : false, true>>;
...
type _S3_IsRowActiveStructOk = Expect<Equal<_S3_IsRowActiveParam extends Incident ? true : false, true>>;
    ...
    columns: [
      {
        accessorFn: (row) => {
          ...
          type _S9_RowExtendsIncident = Expect<Equal<_S9_RowActual extends Incident ? true : false, true>>;
          type _S9_IncidentExtendsRow = Expect<Equal<Incident extends _S9_RowActual ? true : false, true>>;
...
type _S9_Data_ExtendsIncidentArr = Expect<Equal<_S9_DataActual extends (Incident[] | undefined) ? true : false, true>>;
type _S9_Data_IncidentExtends = Expect<Equal<(Incident[] | undefined) extends _S9_DataActual ? true : false, true>>;
...
type _S9_IsRowActiveStructOk = Expect<Equal<_S9_IsRowActiveParam extends Incident ? true : false, true>>;
  ...
  (_ui) => ({
    item: {
      ...
      props: (it) => {
        ...
        // Structural ассерт:
        type _S9Nav_ItExtendsNavItem = Expect<Equal<_S9Nav_ItActual extends NavItem ? true : false, true>>;
        type _S9Nav_NavItemExtendsIt = Expect<Equal<NavItem extends _S9Nav_ItActual ? true : false, true>>;
        // Строгий Equal:
...
type ShapeConfigArg<S extends z.ZodType> =
  ...
  | ((props: IDataTableProps<RowOf<S>> & { data?: ShapeData<S> }) => Partial<IDataTableProps<RowOf<S>>>);
...
type _PP_GetRowIdExtendsIncident = Expect<Equal<_PP_GetRowIdParam extends Incident ? true : false, true>>;
type _PP_IncidentExtendsGetRowId = Expect<Equal<Incident extends _PP_GetRowIdParam ? true : false, true>>;
...
type _PP_DataExtendsArr = Expect<Equal<_PP_DataActual extends (Incident[] | undefined) ? true : false, true>>;
type _PP_ArrExtendsData = Expect<Equal<(Incident[] | undefined) extends _PP_DataActual ? true : false, true>>;
...
const _shapePFuncPropsNeg = ShapeP(
  (ui) => ({ schema: z.array(IncidentSchema), as: DataTableComp }),
  (props) => {
    type _PP_PropsIsDataTable = Expect<Equal<typeof props extends IDataTableProps<Incident> ? true : false, true>>;
    return {};
  },
);
...
type _PO_GetRowIdExtendsIncident = Expect<Equal<_PO_GetRowIdParam extends Incident ? true : false, true>>;
type _PO_IncidentExtendsGetRowId = Expect<Equal<Incident extends _PO_GetRowIdParam ? true : false, true>>;
type _PO_GetRowIdNotAny = Expect<Equal<IsAny<_PO_GetRowIdParam>, false>>;

// --- P-2c: негатив в объекте — row.nonExistent → @ts-expect-error ---
const _shapePObjNeg = ShapeP(
  (ui) => ({ schema: z.array(IncidentSchema), as: DataTableComp }),
  {
    columns: [
      {
