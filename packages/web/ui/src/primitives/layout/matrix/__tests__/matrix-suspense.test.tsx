...
describe('Matrix per-slot Suspense — structural presence in source', () => {
  it('matrix.tsx imports Suspense from solid-js', () => {
    const fs = require('node:fs');
    const path = require('node:path');
    const src = fs.readFileSync(path.resolve(__dirname, '../matrix.tsx'), 'utf-8');
  ...
  it('matrix.tsx has Suspense wrapping in renderCell plain branch', () => {
    const fs = require('node:fs');
    const path = require('node:path');
    const src = fs.readFileSync(path.resolve(__dirname, '../matrix.tsx'), 'utf-8');
    // All three render paths wrap content in <Suspense>
    const suspenseCount = (src.match(/<Suspense\b/g) ?? []).length;
    // Expect at least 4 Suspense instances:
    // - DnD branch (renderCell)
    // - settings-strip branch (renderCell)
    // - plain branch (renderCell)
    // - packing row (renderPackingRow)
    // - centroid shortcut (MatrixContent.renderContent)
    expect(suspenseCount).toBeGreaterThanOrEqual(4);
  });

  it('matrix.tsx Suspense uses MatrixCellFallback as default', () => {
    const fs = require('node:fs');
    const path = require('node:path');
    const src = fs.readFileSync(path.resolve(__dirname, '../matrix.tsx'), 'utf-8');
