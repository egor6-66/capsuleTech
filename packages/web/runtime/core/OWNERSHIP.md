...
- **NOTE-канон (после ДВУХ идентичных инцидентов — Image/Avatar `45dbf7ed`, Prose 2026-07-05):** новый manifest-тип в `@capsuletech/web-ui` = в ТОМ ЖЕ цикле провести wiring здесь: (1) `ui-kit/imports.tsx` — lazy `export const X`; (2) `wrappers/interfaces.ts` — `import type` + `typeof X` в `ViewUiRaw` + `WidgetUiRaw`; (3) `tsconfig.base.json` path `@capsuletech/web-ui/<subpath>` (shared infra → эскалация architect'у). guard `manifest-path-invariant` ловит пропуск на pre-push (красный `ui.X не резолвится`), но дешевле не запускать цикл — kit-манифест без core-wiring блокирует push всей волны.
...
- [x] **`Prose` добавлен в `Ui` namespace** — lazy import из `@capsuletech/web-ui/prose` (kit-манифест `ui.Prose`, typography-контейнер для rendered-markdown, не critical-path → lazy как Textarea/Image). Plain callable (isLeaf, без sub-components). `typeof Prose` добавлен в `ViewUiRaw` и `WidgetUiRaw`. Закрывает красный guard `manifest-path-invariant` (`ui.Prose` не резолвился) — блокер pre-push волны `feat/wave-image-lessons`. tsconfig path `@capsuletech/web-ui/prose` добавлен architect'ом. 4 новых характеризационных теста в `ui-meta-props.test.tsx`. **ВТОРОЙ идентичный инцидент** класса Image/Avatar → см. NOTE-канон в Quirks (2026-07-05).
...
- [x] **`Source`, `Layer`, `Terrain`, `Sky`, `TerrainPreset`, `BuildingsPreset` добавлены в `Ui` namespace** — 6 lazy imports из `@capsuletech/web-map` (PR #183, 2026-05-28). Все plain callables, без sub-components. `typeof X` добавлен в `ViewUiRaw` и `WidgetUiRaw`. 24 новых характеризационных теста в `src/wrappers/__tests__/ui-meta-props.test.tsx`. 265 тестов green.
- [x] **`Marker` добавлен в `Ui` namespace** — lazy import из `@capsuletech/web-map` (generic `<T>` data prop + onClick handler). Plain callable, без sub-components. `typeof Marker` добавлен в `ViewUiRaw` и `WidgetUiRaw`. 4 новых характеризационных теста в `src/wrappers/__tests__/ui-meta-props.test.tsx`. 269 тестов green (2026-05-28).
- [x] **`MapView` реорганизован в compound** — `Source`/`Layer`/`Terrain`/`Sky`/`Marker`/`TerrainPreset`/`BuildingsPreset` теперь sub-components `MapView` через `Object.assign` (как `Card`, `Dropdown`). API: `<Ui.MapView.Sky />`, `<Ui.MapView.Source />` и т.д. Семантически корректно — все эти компоненты потребляют `MapContext` и должны рендериться внутри `<Ui.MapView>`. Тип в `ViewUiRaw`/`WidgetUiRaw`: `MapView: typeof MapView & { Source, Layer, Terrain, Sky, Marker, TerrainPreset, BuildingsPreset }`. `WithMetaProps` покрывает sub-components через `StaticProps<T>` рекурсию. 28 flat тестов заменены на 28 compound тестов (`Ui.MapView.X`). 269 тестов green (2026-05-28).
