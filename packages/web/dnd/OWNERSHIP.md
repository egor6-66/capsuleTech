...
| Export | Что |
|---|---|
| `createEmittingDroppable(opts)` | droppable с emit через useEmit. opts расширяет IDroppableOptions: `emits: { onDrop?, onDragOver? }` |
| `createEmittingDraggable(opts)` | draggable с emit через useEmit. opts расширяет IDraggableOptions: `emits: { onDragStart?, onDragEnd? }` |
...
Subpath зависит на `@capsuletech/web-core` (useEmit). Generic-ядро (`.`) — framework-agnostic, web-core не тянет.
...
- [x] **`/controllers` subpath — HCA-прослойка (ADR 032, фаза 4, 2026-06-04)** — `src/controllers/`: `createEmittingDroppable` (onDrop + onDragOver emit) + `createEmittingDraggable` (onDragStart + onDragEnd emit). Multi-entry build (vite.config.mts), package.json exports `./controllers`, dep `@capsuletech/web-core`. 9 unit tests. tsconfig.base.json alias — запрошен у главного.
...
| Unit | `src/__tests__/controllers.test.tsx` (9/9) | ADR 032: emit onDrop c { data, pointer, dropInfo }; emit onDrop + оригинальный callback; нет emit без ключа; emit onDragOver реактивно; emit onDragStart с payload; emit onDragEnd с data; нет emit для другого draggable-id; useEmit-scope-error; payload shape |
