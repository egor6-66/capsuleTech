 ...
 *
 * Tests cover what is importable from pure-TS entry points: interface structural
 * contracts, the ColumnDef re-export and the IColumn typed wrapper.
 * Visual + interactive coverage lives in dataTable.stories.tsx.
 * DOM render coverage (createSolidTable smoke inside the composite) requires
 * a Solid JSX transform (vitest + vite-plugin-solid, already configured).
 */
...
describe('IColumn typed wrapper', () => {
  it('accessorKey is constrained to keyof TRow', () => {
...
// ---------------------------------------------------------------------------
// DataTableTemplate HKT marker — compile-time phantom
// ---------------------------------------------------------------------------

describe('DataTableTemplate HKT marker', () => {
  it('DataTableTemplate interface has row and props fields', () => {
    // This is a type-level test — the import just confirms the type exists.
    // Runtime: no-op (interfaces have no runtime representation).
    type T = import('../interfaces').DataTableTemplate;
    // Structural: T has { row: unknown; props: IDataTableProps<unknown> }
    // We can only verify the import doesn't throw.
