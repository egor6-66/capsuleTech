...
- **DnD в дереве (reorder + reparent, creator)** — `tree/useRowDnd.ts` поверх `@capsuletech/web-dnd` (`DnDProvider` в `WebStudio.Provider`). Каждая строка = draggable + droppable; зона (`before`/`after`/`inside`) из вертикальной доли курсора (`dndHelpers.zoneFromRatio`), живой индикатор через `useDnD().pointer/overId` (линия before/after, кольцо inside). Корень не draggable (но droppable). Store-оп `moveNode(dragId, targetId, zone)` (creator-слайс): guard'ы — корень неподвижен, не в себя, не в собственное поддерево (цикл), `canAcceptChild` для родителя назначения. `useRowDnd` деградирует в no-op вне `DnDProvider` (standalone/тесты, паттерн `useStudioMode`). DnD → `moveNode` → document → канвас (verified в браузере: reorder двух Button меняет порядок в канвасе). **Фидбек:** кастомный drag-overlay `providers/DragChip.tsx` (иконка + название узла у курсора через `<DragOverlay>`; дефолтный ghost выключен `overlayMode="none"`); индикатор вставки — жирная primary-линия с точкой (before/after) + fill-кольцо `bg-primary/10 ring-primary` (inside).
...
| `tree/__tests__/dndHelpers.test.ts` | isSelfOrDescendant (self/потомок/не-потомок), zoneFromRatio (container vs leaf пороги) |
