...
**Единый document-store (`src/document.ts`, `useDocument`) — SSOT редактируемого дерева (iter1, бриф `studio-creator-tree-iter1`).** Слил раздельные `selection.ts` (preview пресета) + `composition.ts` (ручная сборка) в ОДИН стор — это устранило «два флоу». Режим (`store` / `creator`) теперь только authoring-поверхность над одной моделью `{ schema, selectedNodeId, loadedPresetId }`. Ops: `loadPreset` (store-replace + provenance), `insertPreset(preset, parentId?)` (creator, append + remap-id, unguarded — accept-гейт в UI), `selectNode`, `patchProps`/`patchNodeType` (granular), `removeNode` (subtree, root-guard по актуальному `schema.components.root`), `reset`. Selectors: `schema`/`selectedNodeId`/`selectedNode`/`loadedPresetId`. Rightbar (`Props`/`Info`) читает `selectedNode()` идентично в обоих режимах; канвас (`CanvasBinding`) рисует весь `schema()` mode-agnostic (один `createEffect`, без null-guard). Экспортится из корневого barrel (`useDocument`, `IWebStudioDocument`, `COMPOSITION_ROOT_ID`).
...
- **Мини-палитра вставки в узле** (`tree/NodePalette.tsx`, creator-mode): клик «＋ добавить» в узле-контейнере раскрывает пресеты, которые узел может принять; клик → `insertPreset(preset, nodeId)`. DnD в iter1 нет (клики). `TreeRow` рендерит узел как контейнер по `acceptsChildren(type)` (принимает детей по манифесту), а не по «уже есть дети» — иначе пустой контейнер не получил бы первую вставку.
- **Accept-гейт — реальный сигнал манифеста, не хардкод** (`manifests/rules.ts`): `acceptsChildren(type)` = `!manifest.isLeaf`; `presetsForNode(nodeType)` фильтрует пресеты через `canAcceptChild` (`accepts`-предикат). Accept-policy, которую бриф §3 считал «отсутствующей», уже есть в kit-манифестах (`isLeaf`/`accepts`) — временный хардкод-предикат не понадобился (флаг architect'у).
...
| `__tests__/document.test.ts` | единый стор: loadPreset (replace + provenance + immutable clone), insertPreset (parentId/дефолт/remap-id/no-op), selectNode/patchNodeType, removeNode (subtree + selection-reset + root-guard) |
...
| `manifests/__tests__/rules.test.ts` | acceptsChildren (container/leaf/unknown), presetsForNode (accept-фильтрация Flex vs Card) |
| `tree/__tests__/NodePalette.test.tsx` | раскрытие + клик пресета → onInsert + закрытие; fallback без подходящих пресетов |
