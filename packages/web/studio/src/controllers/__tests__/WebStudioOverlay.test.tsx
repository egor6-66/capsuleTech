/**
 * Тесты WebStudioOverlay.
 *
 * Проверяем:
 *  1. Chrome (box-shadow/pointer-events/background) по IWebStudioCtx из useCtx
...
const mount = (editorCtx: IWebStudioCtx, nodeId = 'node-1') => {
  // WebStudioOverlay читает ctx.store.ctx.data (XState кладёт schema.context в context.data)
  _mockCtx = { store: { ctx: { data: editorCtx } }, controller: {}, state: {} };
  const container = document.createElement('div');
  document.body.appendChild(container);
  render(() => <WebStudioOverlay nodeId={nodeId} node={makeNode()} />, container);
  return container;
};
...
describe('WebStudioOverlay — chrome по ctx', () => {
...
describe('WebStudioOverlay — emit onSelect при клике', () => {
...
describe('WebStudioOverlay — линия вставки', () => {
...
describe('WebStudioOverlay — цветная метка', () => {
