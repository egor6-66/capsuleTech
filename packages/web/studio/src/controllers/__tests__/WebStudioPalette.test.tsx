 ...
 * useWebStudioKit используется ТОЛЬКО для контент-registry (Renderer preview).
 */
...
vi.mock('../WebStudioProvider', () => ({
  useWebStudioKit: () => _mockKit,
}));

// Импорт ПОСЛЕ mock'а
const { WebStudioPalette, CATEGORY_LABELS, CATEGORY_ORDER, CONTAINER_ORDER, catRank, orderRank } =
  await import('../WebStudioPalette');
...
const mount = () => {
  ...
  render(() => <WebStudioPalette />, container);
  return container;
};
...
describe('WebStudioPalette — секции из манифестов', () => {
...
describe('WebStudioPalette — draggable payload', () => {
...
describe('WebStudioPalette — темплейт-превью', () => {
  ...
  it('templatdDraggable имеет source:"palette" и template (IWebStudioTree)', () => {
    mount();
    const trigger = document.querySelector(
      '[data-testid^="templates-trigger-"]',
    ) as HTMLElement | null;
...
describe('WebStudioPalette — ContainerItem (composite-части)', () => {
...
describe('WebStudioPalette — контент-kit передаётся в registry для Renderer', () => {
  it('контент-kit из useWebStudioKit попадает в registry.ui Renderer', () => {
    ...
    if (_rendererCalls.length > 0) {
      ...
      // registry.ui должен быть контент-kit (тот же объект, что вернул useWebStudioKit)
