...
vi.mock('../WebStudioOverlay', () => ({
  WebStudioOverlay: () => null,
}));

// Импорт ПОСЛЕ mock'а
const { WebStudioCanvas } = await import('../WebStudioCanvas');
...
const mount = (ctx: IWebStudioCtx) => {
  ...
  render(() => <WebStudioCanvas />, container);
  return container;
};
...
describe('WebStudioCanvas — render structure', () => {
  ...
  it('передаёт WebStudioOverlay в editOverlay', async () => {
    mount(makeEditorCtx());
    const { WebStudioOverlay } = await import('../WebStudioOverlay');
    expect(_rendererCalls[0].editOverlay).toBe(WebStudioOverlay);
  });
});

describe('WebStudioCanvas — placeholder', () => {
...
describe('WebStudioCanvas — onDrop emit', () => {
...
describe('WebStudioCanvas — onSelect при клике по пустому месту', () => {
  it('не эмитит onSelect при клике на дочерний элемент (bubbling от child)', () => {
    ...
    // клик пузырится от WebStudioOverlay → до drop-zone div, но target != currentTarget
...
describe('WebStudioCanvas — createEmittingDroppable вызывается', () => {
