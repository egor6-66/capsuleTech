/**
 * WebStudioController — HCA-Controller для визуального редактора (ADR 032, фаза 5).
 *
 ...
 * `<Controllers.WebStudio>` и читает `useCtx().store.ctx` через дочерние Widget'ы.
 *
...
import type { IWebStudioTree, NodeId } from '../state/types';

// ── Типы контекста ────────────────────────────────────────────────────────────

export interface IWebStudioCtx {
  tree: IWebStudioTree;
...
const WebStudioController = Controller((_services) => {
  const initialCtx: IWebStudioCtx = {
  ...
  return {
    ...
    states: {
      idle: {
         ...
         * payload: IWebStudioTree
         */
        onSetTree({ target, store }) {
          store.update({ tree: target.payload as IWebStudioTree });
        },

        /**
         * onUpdateNodeProps — обновить пропсы одной ноды (shallow merge).
         * Используется WebStudio.Inspector для редактирования пропсов выбранного узла.
...
export default WebStudioController;
