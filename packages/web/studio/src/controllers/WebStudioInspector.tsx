/**
 * WebStudio.Inspector — инспектор пропсов выбранного узла (ADR 032, фаза 6, чанк 4).
 *
 * Портировано из `apps/ui-creator/src/widgets/inspector.tsx` с расширениями:
 *  - `ctx.store.ctx as IWebStudioCtx` → `useWebStudio()` (без кастов);
 ...
 *  - мутации через `useEmit('onUpdateNodeProps', ...)` → WebStudioController.onUpdateNodeProps.
 *
 * Показывает:
 ...
 *
 * Нет внешних props — всё из useWebStudio() + useEmit().
 */
...
import type { IOnUpdateNodePropsPayload } from './WebStudioController';
...
/**
 * WebStudio.Inspector — монтируется внутри `<WebStudio.Provider>`.
 *
 * Читает selectedId + tree через `useWebStudio()`.
 * Эмитит `onUpdateNodeProps` при изменении любого поля.
 */
export const WebStudioInspector = () => {
  const ws = useWebStudio();
  const emit = useEmit();

  const node = () => {
    const id = ws.selectedId;
    return id ? (ws.tree.nodes[id] ?? null) : null;
  };
  ...
  /** Обработчик изменения поля — эмитит onUpdateNodeProps в WebStudioController. */
