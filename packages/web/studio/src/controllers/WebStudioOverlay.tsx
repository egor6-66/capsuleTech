/**
 * WebStudioOverlay — edit-decoration компонент для одной ноды (ADR 032, фаза 5).
 *
 * Монтируется рендерером (`<Renderer editOverlay={WebStudioOverlay} />`) ВНУТРИ
 * бокса каждой ноды (`position:absolute; inset:0`). Размер = размер ноды из CSS,
 * без замеров и ResizeObserver.
 *
 * Читает состояние через `useWebStudio()` (без кастов).
 ...
 * Исходник имел прямые вызовы `editor.setSelectedId()` через `useWebStudio()` —
...
/**
 * WebStudioOverlay — компонент, который app передаёт в
 * `<Renderer editOverlay={WebStudioOverlay} />`.
 *
 * Принимает `{ nodeId, node }` по контракту `IEditOverlayProps` (ADR 031).
 * Читает editor-state через `useWebStudio()` (без кастов).
 * Эмитит `onSelect` через `useEmit`.
 */
export const WebStudioOverlay = (p: IEditOverlayProps) => {
  const { nodeId, node } = p;

  // ControllerContext доступен, т.к. WebStudioOverlay монтируется рендерером
  // в том же owner-tree, где уже живёт <Controllers.WebStudio> (app-Widget).
  const ws = useWebStudio();
  const emit = useEmit();

  /** Цвет ноды: метка доминирует над primary. */
  const color = (): string => ws.marks[nodeId] ?? 'var(--primary)';
  const marked = (): boolean => ws.marks[nodeId] != null;
  const hasDrag = (): boolean => ws.dragSpec != null;
  ...
  const ringStyle = (): string | undefined => {
    ...
    const t = ws.tree;
    if (s) {
      if (ws.dropTargetId === nodeId) return `inset 0 0 0 2px ${c}`;
      if (canInto(t, s, nodeId)) return `inset 0 0 0 1px ${c}`;
      if (marked()) return `inset 0 0 0 1px ${c}`;
      return undefined;
    }
    if (ws.selectedId === nodeId) return `inset 0 0 0 2px ${c}`;
    if (marked()) return `inset 0 0 0 1px ${c}`;
    return undefined;
  };
  ...
  const bgStyle = (): string | undefined => {
    ...
    const s = ws.dragSpec;
    if (s) {
      if (ws.dropTargetId === nodeId) return fill(c, 12);
      if (marked()) return fill(c, 6);
      return undefined;
    }
    if (ws.selectedId === nodeId) return fill(c, 14);
    if (marked()) return fill(c, 6);
    return undefined;
  };
  ...
  const insertionIsRow = (): boolean => {
    const it = ws.intent;
    if (!it) return false;
    const parentNode = ws.tree.nodes[it.parentId];
