...
- **Last activity:** 2026-07-04 — (1) swap-DnD root-fix пакет (см. Quirks); (2) resize per-slot override поверх web-ui `handleActive` (a08ccd8e) + divider-модель `bordered` (слоты = общее пространство с hairline-разделителями, не карточки).
...
- **`bordered` = divider-модель (2026-07-04; ghost-ручки — того же дня, вторая итерация).** Слоты — общее пространство, разделённое hairline-линиями, НЕ карточки: у ячеек нет полного бордера и `rounded-sm`. Divider (`border-l`/`border-t` `border-border/60`) рисуется позиционно (у правого/нижнего соседа пары) когда пара bordered (either-rule: хотя бы один сосед резолвится true). **Линии — исключительно функция `bordered`**: все Resizable матрицы идут с `handleVariant="ghost"` (web-ui, бриф `web-ui-resizable-ghost-handle.md`) — ручка не рисует свою линию ни активная, ни выключенная (только хит-зона + grip-бэйдж). Первая итерация «активная ручка рисует линию + подавление divider'а» отменена по фидбеку user (у Matrix своя бордер-система, линия ручки её дублировала). Вычисление — `dividerBetweenCells`/`dividerBetweenRows` в `utils.ts`. Packing/grid-зоны (insert-канвас) сохраняют карточную модель плиток — осознанно.
