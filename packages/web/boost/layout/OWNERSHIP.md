...
- **`bordered` = divider-модель (2026-07-04).** Слоты — общее пространство, разделённое hairline-линиями, НЕ карточки: у ячеек нет полного бордера и `rounded-sm`. Divider (`border-l`/`border-t` `border-border/60`) рисуется позиционно (у правого/нижнего соседа пары) когда: пара bordered (either-rule: хотя бы один сосед резолвится true) И между парой не рисуется активная resize-ручка — её `bg-border` hairline сам служит разделителем (двойной линии не бывает). Вычисление — `dividerBetweenCells`/`dividerBetweenRows` в `utils.ts`. Packing/grid-зоны (insert-канвас) сохраняют карточную модель плиток (rounded + full border) — осознанно. NB: `useResizeMode()` глобально дефолтится в `true` → в свежем аппе разделителем обычно служит именно линия активной ручки.
