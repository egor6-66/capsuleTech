...
// ---------------------------------------------------------------------------
// IFieldDef structural contracts
// ---------------------------------------------------------------------------

describe('IFieldDef structural contracts', () => {
  ...
  it('accepts accessorFn only (id required for stable key)', () => {
    ...
    const field: IFieldDef<IIncident> = {
      id: 'applicantName',
  ...
  it('all fields (accessorKey, accessorFn, header, cell, id) are individually optional except header', () => {
    ...
    const minimal: IFieldDef<IItem> = { header: 'Name' };
...
describe('resolveValue', () => {
  ...
  it('returns undefined when neither accessor is provided', () => {
    const field: IFieldDef<IIncident> = { header: 'No accessor' };
  ...
  it('handles falsy values (0, empty string) without coercing to undefined', () => {
    ...
    const countField: IFieldDef<IItem> = { accessorKey: 'count', header: 'Count' };
    const labelField: IFieldDef<IItem> = { accessorKey: 'label', header: 'Label' };
...
describe('fieldKey', () => {
  it('returns accessorKey when id is absent', () => {
    ...
    const field: IFieldDef<IItem> = { accessorKey: 'id', header: 'ID' };
    expect(fieldKey(field)).toBe('id');
  });

  it('returns id when both id and accessorKey are present (id wins)', () => {
    type IItem = { id: number };
    const field: IFieldDef<IItem> = { id: 'customId', accessorKey: 'id', header: 'ID' };
  ...
  it('returns undefined when neither id nor accessorKey is provided', () => {
    type IItem = { x: number };
    const field: IFieldDef<IItem> = { header: 'No key' };
