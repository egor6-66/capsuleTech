...
export const toggleTrackCva = cva(
  'group relative inline-flex items-center rounded-full border border-border bg-muted transition-colors duration-200 cursor-pointer disabled:cursor-not-allowed disabled:opacity-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring data-[checked]:bg-primary data-[checked]:border-primary',
  {
    variants: {
      size: {
        sm: 'h-4 w-7',
...
export const toggleThumbCva = cva(
  'block rounded-full bg-background shadow-md transition-transform duration-200 translate-x-0.5',
  {
    variants: {
      size: {
...
export const toggleLabelCva = cva(
  'select-none cursor-pointer text-foreground transition-colors duration-200',
  {
    variants: {
      size: {
        sm: 'text-xs',
