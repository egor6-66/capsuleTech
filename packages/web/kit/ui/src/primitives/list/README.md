...
## List.Item — selectable leaf {#item}

Выбираемая строка списка (`Ui.List.Item`) — stateless, keyboard-accessible «picker option». Единый leaf-примитив за list-/accordion-палитрами (напр. палитра компонентов studio). Подсветка (hover / selected / focus) запечена внутри — потребитель остаётся props-only, ноль сырых классов.
...
<List>
  <For each={presets}>
    {(p) => (
      <List.Item
        icon={p.icon}                       {/* опц. лидирующая иконка (Component) */}
        selected={p.id === activeId}         {/* подсветка bg-accent */}
        trailing={<Badge>{p.count}</Badge>}  {/* опц. правый слот */}
        onSelect={() => pick(p.id)}          {/* click / Enter / Space */}
      >
        {p.label}
      </List.Item>
    )}
...
| Prop | Type | Назначение |
|---|---|---|
| `children` | `JSX.Element` | Лейбл строки |
| `icon` | `Component` | Опц. лидирующая иконка (рендерится `size-4 shrink-0`) |
| `selected` | `boolean` | Подсветка + `aria-selected` + `data-selected` |
| `onSelect` | `() => void` | Клик / Enter / Space |
| `trailing` | `JSX.Element` | Опц. правый слот (badge, счётчик, chevron) |
| `class` | `string` | Доп. классы на кнопке |

`role="option"` + `aria-selected` моделируют одну опцию внутри listbox-подобного контейнера; группировку даёт родительский `List` / `Accordion`. Прочие button-атрибуты (`data-*`, `title`, `aria-*`) проходят насквозь. Тултип-превью — композиция потребителя (`<Tooltip><List.Item/></Tooltip>`), НЕ часть примитива.
