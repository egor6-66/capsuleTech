...
(h1/h2/h3/p/blockquote/code; lead и overline рендерятся как <p>). Поддерживает цветовой override через
tone, выравнивание (align), размер поверх варианта (size), вес шрифта поверх варианта (weight),
моноширинное семейство (mono) и opacity-скрытие без layout shift (dim).
...
|---|---|---|---|
| variant | enum h1/h2/h3/p/blockquote/code/lead/muted/overline | p | Семантический вариант. Определяет HTML-тег и стиль. |
...
| weight | enum thin/light/normal/medium/semibold/bold/extrabold | — | Вес шрифта поверх варианта (выигрывает у веса variant'а). |
| mono | boolean | false | true → font-mono (моноширинное семейство, ортогонально variant/weight/size). |
...
- overline — надзаголовок (eyebrow/kicker). text-xs + uppercase + tracking-widest + muted. Ставится над заголовком секции. Тег p.
...
weight перебивает font-weight, заданный вариантом, сохраняя остальные стили. tailwind-merge
отбрасывает вес варианта в пользу weight (h1 extrabold + weight=normal → normal). Значения:
thin/light/normal/medium/semibold/bold/extrabold.

## Mono — моноширина {#mono}

mono={true} добавляет font-mono. Ортогонален variant/weight/size — меняет только шрифтовое
семейство. Для inline технических токенов (пути, хэши, команды) без chrome варианта code
(bg-muted / padding). Пример: variant=p mono для GET /api/users в потоке текста.
...
variant/color/tone/align/size/weight, boolean для mono/dim, examples (palette preview), styleSlots: [root].
