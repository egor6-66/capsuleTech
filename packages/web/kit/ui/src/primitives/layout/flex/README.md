...
<!-- audience: agent -->
`flex.contract.ts` описывает CSS-flex props + sizing в zod-схеме для studio inspector: `orientation`, `direction`, `wrap`, `align`, `justify`, `gap`, `gapX`, `gapY`, `inline`, sizing-шкала. `rule.styleSlots(['root'])` — одна корневая зона стилей.
