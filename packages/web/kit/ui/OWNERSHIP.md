...
- **Last activity:** 2026-07-06 (`Ui.Article` composite — article-renderer: заголовок+лид+markdown-тело(узел-слот)+примеры(сущностный Card)+связанные-чипы(Badge interactive) + manifest `ui.Article` — brief article-1-ui; ранее — `Ui.Card` data-driven сущностный режим — слоты + пресеты + вшитый a11y — brief entity-card-1-ui; ранее — `SectionedList` composite + manifest `ui.SectionedList`/`ui.Accordion`, Accordion `segmented`/`density`/`nested`/`subtitle`, `List.Item`, SegmentedBar + Launcher).
...
`./button`, `./card`, `./field`, `./flex`, `./grid`, `./input`, `./label`, `./layout` (deprecated alias на matrix), `./list`, `./matrix`, `./select`, `./separator`, `./slot`, `./table`, `./textarea`, `./toggle`, `./tooltip`, `./typography`, `./wrappers`, `./dataTable`, `./previewCard`, `./sectionedList`, `./segmentedBar`, `./launcher`, `./article`, `./image`, `./avatar`.
...
- **Единый border-токен (2026-07-05, бриф `matrix-border-1-ui.md`).** Активная ручка рисует hairline единым токеном `bg-border` (как Card/Input в ките — «все бордеры одинаковы»), НЕ `border/60`. Прошлый `handleVariant: 'line' | 'ghost'` (бриф `web-ui-resizable-ghost-handle.md`) **снят** — новая модель Matrix инвертировалась (на resizable-стыке ручка И ЕСТЬ divider, Matrix гасит свою сторону), поэтому «ручка без линии» больше не нужна: ручка всегда рисует стандартный hairline. Ось `variant` и compoundVariants удалены из `resizableHandleCva`; `active:true` теперь несёт `bg-border` напрямую. Landed cross-package с `matrix-border-2-boost-layout.md`.
...
### SegmentedBar + Launcher — stateless nav-примитивы (2026-07-05)

По брифу `docs/_meta/briefs/pilot-segment-nav-2-ui.md` (пилот дедупа Nav/Welcome, канон [[feedback_product_wide_kit_layering]]) — два stateless, пресет-driven composite'а, визуал вынесен из learn-копий. Роутер/emit не знают (connected-слой = `@capsuletech/web-shell`, brief 3/4). Классы легитимны только внутри компонента; consumer — props/пресеты, ноль сырых классов.

- **`SegmentedBar`** (`composites/segmentedBar/`, subpath `@capsuletech/web-ui/segmentedBar`) — сегмент-бар из learn `library/Navigation.tsx`. `Group attached` → `For items` → `Button` (active=primary / inactive=ghost) + `aria-current="page"` + `pointer-events-none` на активном. Props: `items`, `activeId?`, `onSelect`, `preset?`, `class?`/`style?`. Активный = `item.id === activeId`.
...
### SectionedList composite + manifest-регистрация (2026-07-05)

По брифу `sectioned-list-1-ui.md` (канон [[feedback_compositions_in_kit_for_store_renderer]] — любая визуальная композиция живёт СТРОГО в web-ui пресетом, иначе мимо стора/рендерера studio). Landing парой с `sectioned-list-2-web-core.md` (инвариант-тест `manifest-path-invariant` в web-core связывает `manifest.type` ↔ Ui-namespace — врозь CI красный).

- **`SectionedList`** (`composites/sectionedList/`, subpath `@capsuletech/web-ui/sectionedList`) — «аккордеон групп → выбираемый список». Единый kit-дом для паттерна learn `Concepts`/`Rules` + studio `ComponentSegments`. Внутри: `<Accordion multiple>` **studio-look (без `bordered`/`rounded`)** → на секцию `Accordion.Item` → `Accordion.Trigger subtitle={s.subtitle}` (+ иконка слева) → `<List>` из `List.Item` (`selected`/`onSelect`). `itemPreview?` (runtime-only, studio) → кит оборачивает каждый ряд в `<Tooltip>` (`Tooltip.Trigger as="div"` — leaf-`<button>` не вложен в button). Open: `open`/`onOpenChange` → controlled, иначе внутренний сигнал seed из `defaultOpen` (`'all'` = все секции). Данные — plain/сериализуемые (`label` рантайм-JSX, в сторе — string). Ноль сырых классов наружу.
- **Manifest-регистрация (часть B, дыра закрыта):** `sectionedList.manifest.tsx` — `ui.SectionedList` (category `composite`, isLeaf, propsSchema: sections/selectedId/open с label-строками для инспектора, `defaultProps` сэмпл-секции, preset `reference`). `accordion.manifest.tsx` (раньше Accordion В МАНИФЕСТЕ НЕ БЫЛО) — `ui.Accordion` (container, presets `plain`/`segmented`/`subtitle`) + compound-части `ui.Accordion.Item`/`.Trigger`/`.Content` (по образцу Card). Обе записи + части добавлены в `manifest/registry.ts` `ALL[]`. `getAllManifests()` теперь содержит `ui.SectionedList` + `ui.Accordion` (+ parts) — проверено тестом.
- Тесты: `composites/sectionedList/__tests__/sectionedList.test.tsx` (рендер секций+subtitle, `defaultOpen="all"` раскрывает все, collapsed по дефолту, ряды = `role=option`, `selectedId` подсветка, click→onSelect, itemPreview не ломает интеракцию, manifest-регистрация SectionedList+Accordion+parts+reference-preset — 10). Stories: `Default`/`SingleOpen`/`WithItemPreview`/`ControlledOpen`.
- **Зависимость на web-core:** инвариант-тест `manifest-path-invariant` (в web-core) требует, чтобы `ui.Accordion`/`ui.Accordion.*`/`ui.SectionedList` резолвились в namespace `Ui` (`web-core/src/ui-kit/imports.tsx`). Пока owner-web-core не добавит их (парный бриф) — их тест ляжет; это **ожидаемо** и не влияет на web-ui build/test.

### Article composite + manifest-регистрация (2026-07-06)

По брифу `article-1-ui.md` (канон [[feedback_compositions_in_kit_for_store_renderer]] / [[component-model]] — любая визуальная композиция живёт СТРОГО в web-ui пресетом). Схлопывает hand-composed learn `Concept` (статья строилась руками: `Layout.Flex` + `Typography h1` + `Markdown` + `For` примеры + `For` чипы = антипаттерн) в data-driven composite.

- **`Article`** (`composites/article/`, subpath `@capsuletech/web-ui/article`) — «заголовок + лид + markdown-тело + примеры + связанные-чипы». Вертикальный стек (`Flex` vertical), каждый слот `<Show>`-gated (отсутствие = погашен). `title` → `Typography h1`; `lead` → `Typography tone="muted" size="lg"`; `body` → **узел-слот** (`JSX.Element`, потребитель передаёт свой отрендеренный markdown — кит только позиционирует; вынос `Markdown`/wikilink в кит = отдельный будущий заход); `examples` → стек сущностных `Card padding="sm"` (`primary`→title, `secondary`→subtitle); `related` → заголовок `relatedLabel` + wrap-ряд `Badge interactive onClick→onRelatedSelect(id)`. Ноль сырых классов наружу. Роутер/emit не знает — клик по чипу уходит через `onRelatedSelect` (connected-обвязка = доменная фича аппа).
- **Manifest-регистрация:** `article.manifest.tsx` — `ui.Article` (category `composite`, `isLeaf`, `canBeRoot`; propsSchema: title/lead/examples/related/relatedLabel как сериализуемые строки для инспектора; `body`/`onRelatedSelect` — runtime-only, вне схемы; `defaultProps` + preset `concept` сэмпл-статья). Добавлен в `manifest/registry.ts` `ALL[]`. `getAllManifests()` содержит `ui.Article`.
- Тесты: `composites/article/__tests__/article.test.tsx` (title+lead, body node-slot, examples→entity cards, related→interactive chips role=button, click→onRelatedSelect, пустые/empty-array слоты скрыты, manifest-регистрация + concept-preset — 9). Stories: `Full`/`TextOnly`/`ExamplesOnly`/`RelatedOnly`. README + subpath exports (`./article` + `/*`).
- **Зависимость на web-core:** `Ui.Article` в namespace (`web-core/src/ui-kit/imports.tsx`) + инвариант-тест `manifest-path-invariant` — парный бриф (architect делает параллельно). Пока не добавлен — invariant-тест в web-core ляжет; это ожидаемо, на web-ui build/test не влияет.

### Card — data-driven сущностный режим + вшитый a11y (2026-07-05)

По брифу `entity-card-1-ui.md` (канон [[component-model]] — карточка = **сущность**, рисует данные по ключам; вид = пресет, полнота = слоты вкл/выкл, точечно = оверрайд пропсами; потребитель НЕ строит лайаут). Схлопывает hand-composed learn-карточки (`Card+Flex+Typography+Badge` руками = антипаттерн) на data-driven `Ui.Card`. Публичный API не сломан — compound-режим (`Card.Header/…` через children) не тронут.

- **Сущностный режим (Часть A):** slot-пропы в `ICardEntityProps` (`interfaces.ts`) — `title`, `titleAction`, `subtitle`, `translation`, `definition`, `badge`, `tags: string[]`, `meta: {key,value}[]`, `align`. Активируется, когда задан любой content-слот (`isEntity()` в `card.tsx`); тогда Card рисует вертикальный стек сам (`entity.tsx` `CardEntityContent`) — строка `title`+`titleAction`(+`badge` через `Ui.Badge`) → `subtitle` → `translation` → `definition` → ряд `tags` (каждый `Ui.Badge tone="muted"`, wrap) → `meta` key:value muted-строки. `<Show>` на каждый слот (отсутствие = слот погашен). `align` (`center`/`start`, default `start`) центрирует стек. Вся структура/классы — в ките, ноль сырых наружу. `ICardProps` extends `Omit<HTMLAttributes, keyof ICardEntityProps>` — слоты выигрывают у HTML-атрибутов (`title` и др.).
- **Padding:** сущностный режим сам даёт внутренний `p-card` (default `padding='md'` когда не задан — нет `Card.Content` чтобы падить). Compound-режим без изменений (root chrome-only, части падят себя).
- **Вшитый a11y (Часть B):** когда `interactive` И есть `onClick` — Card сам ставит `role="button"`, `tabIndex={0}`, Enter/Space → `onClick` (`.click()`-прокси). Явный `role`/`tabIndex` от потребителя выигрывает. Потребитель больше не пишет a11y руками. **Launcher** соответственно снял свой ручной `role`/`tabIndex`/`onKeyDown` (был double-fire) — теперь только `interactive` + `onClick`.
- **Регистрация (Часть C, 1 слой — `Ui.Card` уже в namespace):** `card.manifest.tsx` `propsSchema` расширен сущностными слотами (label'ы как string для инспектора; `tags` array, `meta` array of `{key,value}`, `align` enum). `card.presets.ts` +3 сущностных пресета: `word-compact` (title+titleAction+subtitle+translation, center, selectable), `word-full` (то же + definition+tags+meta, start), `entity-meta` (title+badge+tags, selectable). Каждый = single-node `ui.Card` с сериализуемыми props. manifest-invariant зелёный (Card уже резолвится).
- Тесты: `__tests__/card-entity.test.tsx` (слоты рендер+гейт, tags/badge/meta, align, a11y button/Enter/Space/explicit-role-wins, compound-режим не тронут + реактивный переход в entity — 15). Обновлён `manifest/__tests__/registry.test.ts` (Card теперь 6 пресетов: 3 compound-дерева + 3 entity single-node). Stories: `EntityWordCompact`/`EntityWordFull`/`EntityLessonCard`/`EntitySlotsToggle`.
