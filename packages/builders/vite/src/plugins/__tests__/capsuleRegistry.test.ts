...
describe('generateBootstrap — embed-aware named exports (ADR 059)', () => {
  ...
  it('exports appConfig (consumed by createCapsuleApp in index.ts)', () => {
    const out = generateBootstrap();
...
// ADR 059 Phase 2 — index.ts is now codegen (always-regenerated), not scaffold
// copy-once. This guarantees existing apps pick up the embed-aware entry without
// a manual `.capsule/` wipe (canon §0/§1: no manual-workaround). The standalone
// `createRoot(Bootstrap)` model is retired from the entry.
describe('generateIndexEntry — embed-aware mount entry (ADR 059 Phase 2)', () => {
  it('starts with generated comment (not user-editable)', () => {
    expect(generateIndexEntry().startsWith('// generated by CapsuleRegistryPlugin')).toBe(true);
  });

  it('imports createCapsuleApp from @capsuletech/web-core/bootstrap', () => {
    expect(generateIndexEntry()).toContain(
      "import { createCapsuleApp } from '@capsuletech/web-core/bootstrap';",
    );
  });

  it('imports appConfig + routeTree from generated bootstrap', () => {
    expect(generateIndexEntry()).toContain("import { appConfig, routeTree } from './bootstrap';");
  });

  it('mounts via createCapsuleApp with routeTree + appConfig + basepath', () => {
    expect(generateIndexEntry()).toContain(
...
// ---------------------------------------------------------------------------
// generateRemotesRuntime (ADR 060 Phase 2 / D5)
// ---------------------------------------------------------------------------

describe('generateRemotesRuntime', () => {
  it('emits a {name,url}[] registry from config.remotes', () => {
    const out = generateRemotesRuntime([
      { name: 'canvas', url: 'https://canvas.example.com' },
      { name: 'editor', url: 'https://editor.example.com', contract: '/custom' },
    ]);
    expect(out).toContain('export const remotes = [');
    expect(out).toContain('{ name: "canvas", url: "https://canvas.example.com" }');
    expect(out).toContain('{ name: "editor", url: "https://editor.example.com" }');
    expect(out).toContain('as const;');
    // `contract` — host-side vendoring hint, НЕ часть runtime-реестра.
    expect(out).not.toContain('contract');
  });

  it('empty remotes → valid empty registry', () => {
    const out = generateRemotesRuntime([]);
    expect(out).toContain('export const remotes = [] as const;');
  });

  it('starts with the generated-by header', () => {
    expect(generateRemotesRuntime([]).startsWith('// generated by CapsuleRegistryPlugin')).toBe(
      true,
    );
  });
});
...
// ---------------------------------------------------------------------------
// generateRemotesTypes (ADR 060 Phase 2 / D6)
// ---------------------------------------------------------------------------

describe('generateRemotesTypes', () => {
  // Vendored schema.json shape (per-event json-schema по осям in/out — как Phase 1 buildSchemaJson).
  const canvasSchema = {
    in: { setMarkers: { type: 'array', items: { type: 'string' } } },
    out: {
      mounted: {
        type: 'object',
        properties: { name: { type: 'string' }, ts: { type: 'number' } },
        required: ['name', 'ts'],
      },
    },
  };

  it('ИНЛАЙНИТ типы из vendored schema.json (без import) + export {}', () => {
    const out = generateRemotesTypes(
      [{ name: 'canvas', url: 'https://canvas.example.com' }],
      () => canvasSchema,
    );
    // КЛЮЧЕВОЕ: никаких import-ссылок (ни import type, ни import('path').Type).
    expect(out).not.toContain('import');
    // export {}; обязателен (файл = модуль → declare module = augmentation, не ambient).
    expect(out).toContain('export {};');
    expect(out).toContain("declare module '@capsuletech/web-remote' {");
    expect(out).toContain('interface CapsuleRemotes {');
    // инлайн-структура in/out с типами из schema.
    expect(out).toContain('"canvas": { in:');
    expect(out).toContain('"setMarkers": string[]');
    expect(out).toContain('"name": string');
    expect(out).toContain('"ts": number');
  });

  it('пустая ось → {} (валидный IRemoteContract)', () => {
    const out = generateRemotesTypes([{ name: 'x', url: 'u' }], () => ({ in: {}, out: {} }));
    expect(out).toContain('"x": { in: {}; out: {} };');
  });

  it('skips a remote without vendored schema.json + warns', () => {
    ...
    expect(out).not.toContain('CapsuleRemotes');
    expect(out).toBe(
      '// generated by CapsuleRegistryPlugin — не редактировать руками\nexport {};\n',
    );
    expect(warnings).toHaveLength(1);
    expect(warnings[0]).toContain('capsule remote sync canvas');
  });

  it('augments only the synced subset (mixed)', () => {
    const out = generateRemotesTypes(
      [
        { name: 'canvas', url: 'u1' },
        { name: 'editor', url: 'u2' },
      ],
      (name) => (name === 'canvas' ? canvasSchema : null),
    );
    expect(out).toContain('"canvas":');
    expect(out).not.toContain('"editor":');
  });

  it('no remotes → minimal valid module (export {}, no augmentation)', () => {
    const out = generateRemotesTypes([], () => ({ in: {}, out: {} }));
    expect(out).toContain('export {};');
    expect(out).not.toContain('CapsuleRemotes');
  });
});
