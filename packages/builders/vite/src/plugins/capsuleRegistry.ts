...
export const generateBootstrap = (contributions: readonly BootstrapContribution[] = []): string => {
  ...
  lines.push('');
  // `Bootstrap` STAYS: the legacy `.capsule/remote-entry.ts` (srcdoc/boot model,
  // RemoteManifestPlugin) imports `{ Bootstrap } from './bootstrap'`. It must
  // survive until the host moves to iframe-src (Brief 3), then both retire.
...
// app author. The retired `createRoot(Bootstrap)` model is gone from the entry;
// the `Bootstrap` export it used survives in bootstrap.tsx only for the legacy
// remote-entry (retires with Brief 3).
// ---------------------------------------------------------------------------
