...
// ADR 060 D6 — инлайн json-schema → TS для remotes.d.ts augmentation (тот же рендер,
// что contract.d.ts в Phase 1). Без import-ссылок в augmentation (см. generateRemotesTypes).
import { jsonSchemaToTs } from './contractArtifact';
...
export const generatePackagesTypes = (entries: ResolvedPackageEntry[]): string => {
  ...
  for (const { pkg, globalName, componentKeys } of componentEntries) {
    ...
    lines.push(
      `    // Aggregate of all component events. Use ${globalName}.Comp.Events for per-component granularity.`,
    );
...
// ---------------------------------------------------------------------------
// Sub-generator: remotes (ADR 060 Phase 2 / D5 + D6)
//
//  D5 runtime — .capsule/registry/remotes.ts: реестр { name, url }[] из
//    config.remotes. Provider читает его (web-remote 4-of-4) вместо inline
//    <Remote.Provider modules={...}>. Здесь — только эмит данных.
//
//  D6 типы — .capsule/@types/remotes.d.ts: для каждого remote.name с vendored
//    apps/<app>/remotes/<name>/contract.d.ts генерится augmentation интерфейса
//    `CapsuleRemotes` (определён в web-remote, 4-of-4) → <Remote.View name="...">
//    типизирован без импортов в app-коде. Без vendored-контракта — name
//    пропускается (warn), его View остаётся слабо-типизирован.
// ---------------------------------------------------------------------------
...
/**
 * Generates .capsule/registry/remotes.ts (D5).
 * Stateless: (remotes) => string. Пустой список → валидный пустой реестр.
 */
export const generateRemotesRuntime = (remotes: ReadonlyArray<RemoteEntry>): string => {
  const HEADER = '// generated by CapsuleRegistryPlugin — не редактировать руками';
  if (remotes.length === 0) {
    return [HEADER, 'export const remotes = [] as const;', ''].join('\n');
  }
  const entries = remotes.map(
    (r) => `  { name: ${JSON.stringify(r.name)}, url: ${JSON.stringify(r.url)} }`,
  );
  return [HEADER, 'export const remotes = [', `${entries.join(',\n')},`, '] as const;', ''].join(
    '\n',
  );
};

/**
 * Vendored `apps/<app>/remotes/<name>/schema.json` — per-event json-schema по осям
 * in/out (форма из ADR 060 Phase 1 `buildSchemaJson`; кладёт `capsule remote sync`).
 */
export interface RemoteSchemaJson {
  in?: Record<string, unknown>;
  out?: Record<string, unknown>;
}

/**
 * Generates .capsule/@types/remotes.d.ts (D6).
 * Stateless: (remotes, loadSchema, warn?) => string.
 *
 * `loadSchema(name)` — читает vendored `apps/<app>/remotes/<name>/schema.json`
 * (в плагине — fs; в тестах — mock). Нет файла → `null` → name пропускается + warn.
 *
 * ⚠️ Типы **ИНЛАЙНЯТСЯ** прямо в augmentation, БЕЗ `import`/`import('path').Type`.
 * Причина (architect задиагностировал tsc'ом): в app-tsconfig (`moduleDetection: 'force'`)
 * ЛЮБАЯ cross-module ссылка в augmentation-файле → `declare module` НЕ применяется
 * (View сваливается в loose `any`). Работает только полностью инлайн-форма.
 * `export {};` ОБЯЗАТЕЛЕН — делает файл модулем (иначе `declare module` станет
 * ambient-декларацией, а не augmentation). Источник типов — `schema.json` через
 * `jsonSchemaToTs` (тот же рендер, что Phase 1 contract.d.ts).
 */
export const generateRemotesTypes = (
  remotes: ReadonlyArray<RemoteEntry>,
  loadSchema: (name: string) => RemoteSchemaJson | null,
  warn: (msg: string) => void = () => {},
): string => {
  const HEADER = '// generated by CapsuleRegistryPlugin — не редактировать руками';

  const resolved: Array<{ name: string; schema: RemoteSchemaJson }> = [];
  for (const r of remotes) {
    const schema = loadSchema(r.name);
    if (schema) {
      resolved.push({ name: r.name, schema });
    } else {
      warn(
        `[capsule:remotes] нет vendored schema.json для "${r.name}" — <Remote.View name="${r.name}"> остаётся слабо-типизирован. Запусти \`capsule remote sync ${r.name}\`.`,
      );
    }
  }
  ...
  const renderEventMap = (events?: Record<string, unknown>): string => {
    const keys = events ? Object.keys(events) : [];
    if (keys.length === 0) return '{}';
    const entries = keys.map((k) => `${JSON.stringify(k)}: ${jsonSchemaToTs(events![k])}`);
    return `{ ${entries.join('; ')} }`;
  };
  ...
  for (const { name, schema } of resolved) {
    lines.push(
      `    ${JSON.stringify(name)}: { in: ${renderEventMap(schema.in)}; out: ${renderEventMap(schema.out)} };`,
    );
  }
  lines.push('  }');
  lines.push('}');
...
interface AppConfigShape {
  ...
  /** ADR 060 D5 — host-side реестр встраиваемых ремоут-аппов. */
  remotes?: ReadonlyArray<{ name: string; url: string; contract?: string }>;
}

export const renderAppTagsTypes = (
...
export const generateAppConfigRuntime = (
  ...
  if (hasAccess) {
    lines.push('', 'if (appConfig.access) {', '  setupAccess(appConfig.access, useAuth());', '}');
  }
  if (hasAuthSession) {
    lines.push(
      '',
...
/**
 * A contribution from a sub-generator to the bootstrap.tsx import list.
 */
export interface BootstrapContribution {
  phase: 'globals' | 'subsystems' | 'render';
  importPath: string;
}

/**
 * Generates .capsule/bootstrap.tsx.
 * Stateless: (contributions?) => string.
 *
 * Import ordering:
 *   1. LAYER_INIT_ORDER entries (legacy, always present) — globals first, then
 *      subsystems, then render. render phase entries become named imports.
 *   2. Extra contributions from sub-generators (e.g. docs-sources) — inserted
 *      after LAYER_INIT_ORDER entries of the SAME phase; deduplicated by importPath.
 *
 * Deduplication: if a contribution's importPath matches an existing
 * LAYER_INIT_ORDER entry, it is silently skipped (LAYER_INIT_ORDER wins).
 *
 * Backwards-compatible: calling generateBootstrap() with no arguments produces
 * the same output as before (only LAYER_INIT_ORDER entries).
 *
...
export const generateBootstrap = (contributions: readonly BootstrapContribution[] = []): string => {
  ...
  // Filter contributions: skip those whose importPath already exists in LAYER_INIT_ORDER.
  const filtered = contributions.filter((c) => !existingPaths.has(c.importPath));

  // Group filtered contributions by phase.
  const extraByPhase: Record<'globals' | 'subsystems' | 'render', BootstrapContribution[]> = {
    globals: [],
    subsystems: [],
    render: [],
  };
  for (const c of filtered) {
    extraByPhase[c.phase].push(c);
  }
  ...
  // Emit imports phase by phase to maintain correct load order.
  // Within each phase: LAYER_INIT_ORDER entries first (stable), then contributions (stable order).
  const allPhases: Array<'globals' | 'subsystems' | 'render'> = ['globals', 'subsystems', 'render'];

  for (const phase of allPhases) {
    const legacyEntries = LAYER_INIT_ORDER.filter((l) => l.phase === phase);
    const extraEntries = extraByPhase[phase];

    for (const layer of legacyEntries) {
      if (layer.name === 'routes') {
        // Routes is a named import — routeTree is re-exported for the index.ts
        // entry (createCapsuleApp consumes it).
  ...
  lines.push('');
  // ADR 059 Phase 2: `appConfig` + `routeTree` are exported so the embed-aware
  // entry (index.ts → createCapsuleApp) can consume them. The handshake +
  // config-override path lives entirely in createCapsuleApp (web-core).
  lines.push('export const appConfig = appConfigRaw as IAppConfig;');
  lines.push('export { routeTree };');
  lines.push('');
...
// ---------------------------------------------------------------------------
// Sub-generator: index.ts (mount entry, ADR 059 Phase 2)
//
// The standalone+embedded mount entry. Generated (always-regenerated), NOT a
// scaffold copy-once template — so existing apps pick up the embed-aware entry
// on the next `capsule dev`/`build` without a manual `.capsule/` wipe (canon
// §0/§1: no manual-workaround).
//
// One entry for both modes: `createCapsuleApp` (web-core) detects host-iframe
// (window.parent !== window), runs the postMessage-handshake and merges the
// host config-override BEFORE mount (ADR 059). Embedding is invisible to the
// app author. The retired `createRoot(Bootstrap)` srcdoc/boot model is fully
// gone (ADR 059 Phase 3) — bootstrap.tsx now only re-exports appConfig +
// routeTree for this entry.
// ---------------------------------------------------------------------------

/**
 * Generates .capsule/index.ts — the app mount entry.
 * Stateless: (hasContract?) => string. Deterministic.
 *
 * ADR 060 Phase 3 / D1: если есть `apps/<app>/contract.ts`, entry импортит его
 * default и передаёт `contract` в `createCapsuleApp` — фреймворк форвардит хосту
 * только `out`-события и валидирует входящие host-диспатчи по `in` (web-core 2-of-3).
 * Без contract.ts `contract` не передаётся (опциональный; standalone работает как раньше).
 *
 * `defineContract` в contract.ts — bare-глобал (DEFINE_FACTORIES): когда Vite
 * трансформит `../contract` (он теперь в module-графе через этот импорт), AutoImport
 * инжектит реальный импорт `defineContract`. Здесь мы импортим только вычисленный
 * default — обращения к `defineContract` в entry нет.
 */
export const generateIndexEntry = (hasContract = false): string => {
  const lines: string[] = [
    '// generated by CapsuleRegistryPlugin — не редактировать руками',
  ...
  if (hasContract) {
    // ADR 060 Phase 3 — публичный контракт аппа (defineContract default).
    lines.push("import contract from '../contract';");
  }
  lines.push(
    '',
    '// ADR 059: один entry для standalone и embedded. createCapsuleApp сам детектит',
    '// хост-iframe (window.parent !== window), делает postMessage-handshake и мержит',
    "// host-override в config ДО mount'а. App-разработчику embedding невидим.",
    hasContract
...
const TYPED_LAYERS = new Set<string>([
  'features',
  'controllers',
  'entities',
] satisfies TypedLayer[]);

/**
 * Builds a segment access chain using single-quoted bracket notation.
 * e.g. ['Auth', 'Login'] → "['Auth']['Login']"
 */
const segmentChain = (segments: string[]): string => segments.map((s) => `['${s}']`).join('');
...
const renderNamespaceBody = (segments: string[], typeLine: string, indent: string): string => {
  if (segments.length === 1) {
    ...
    return [`${indent}namespace ${segments[0]} {`, `${indent}  ${typeLine}`, `${indent}}`].join(
      '\n',
    );
  }
...
export const CapsuleRegistryPlugin = ({
  ...
  const indexEntryOut = resolve(capsuleRoot, 'index.ts');
  const docsSourcesOut = resolve(capsuleRoot, 'registry', 'docs-sources.ts');
  const remotesOut = resolve(capsuleRoot, 'registry', 'remotes.ts');
  const remotesTypesOut = resolve(capsuleRoot, '@types', 'remotes.d.ts');
  ...
  // ADR 060 Phase 3 / D1 — публичный контракт аппа (apps/<app>/contract.ts).
  // Если есть — entry прокидывает его default в createCapsuleApp.
  const contractPath = resolve(appConfigDir, 'contract.ts');

  // ADR 060 D6 — читает vendored apps/<app>/remotes/<name>/schema.json
  // (положил `capsule remote sync`). null → нет/битый → name пропускается (warn).
  // Типы инлайнятся из schema.json в augmentation (без import-ссылок).
  const loadRemoteSchema = (name: string): RemoteSchemaJson | null => {
    const p = resolve(appConfigDir, 'remotes', name, 'schema.json');
    if (!existsSync(p)) return null;
    try {
      return JSON.parse(readFileSync(p, 'utf-8')) as RemoteSchemaJson;
    } catch {
      return null;
    }
  };
  ...
  const loadAndFlushAppConfig = () => {
    if (!existsSync(appConfigPath)) {
      ...
      // ADR 060 — пустой remote-реестр без capsule.app.ts.
      writeOut(remotesOut, generateRemotesRuntime([]));
      writeOut(remotesTypesOut, generateRemotesTypes([], loadRemoteSchema));
    ...
    // ADR 060 D5/D6 — remote-реестр (runtime) + remotes.d.ts (типы из vendored-контрактов).
    const remotes = config?.remotes ?? [];
    writeOut(remotesOut, generateRemotesRuntime(remotes));
    writeOut(
      remotesTypesOut,
      generateRemotesTypes(remotes, loadRemoteSchema, (m) => console.warn(m)),
    );
  ...
  const initialScan = async (absWatch: string) => {
    ...
    flushBootstrap();
    // index.ts (mount entry) — always-regenerated codegen (ADR 059 Phase 2),
    // so existing apps pick up the embed-aware entry without a `.capsule/` wipe.
    // Content is static; written once per plugin run, overwriting any stale
    // `createRoot(Bootstrap)` entry from before the migration.
    // ADR 060 Phase 3 — прокидываем contract в entry, если apps/<app>/contract.ts есть.
    writeOut(indexEntryOut, generateIndexEntry(existsSync(contractPath)));
  };
  ...
  return {
    ...
    async configureServer(server: ViteDevServer) {
      ...
      // ADR 060 D6 — watch apps/<app>/remotes/** (vendored-контракты кладёт
      // `capsule remote sync`). Появление/удаление contract.d.ts → регенерим
      // remotes.d.ts (через полный re-eval appConfig — дёшево и без рассинхрона).
      const remotesDir = resolve(appConfigDir, 'remotes');
      server.watcher.add(remotesDir);
      const onRemotesEvent = (file: string) => {
        if (normalizePath(file).includes('/remotes/')) loadAndFlushAppConfig();
      };
      server.watcher.on('add', onRemotesEvent);
      server.watcher.on('unlink', onRemotesEvent);
