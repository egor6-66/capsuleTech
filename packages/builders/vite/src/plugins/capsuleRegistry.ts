...
/**
 * Resolves the globalName for a package entry by loading its /capsule manifest
 ...
 */
const resolveManifestName = (entry: PackageEntry, appConfigDir: string): string | null => {
  if (entry.as) return entry.as;
  ...
  try {
    ...
    const mod = j(manifestPath) as { name?: string } | { default?: { name?: string } } | null;
    const manifest = (mod as { default?: { name?: string } })?.default ?? (mod as { name?: string });
    if (!manifest?.name) {
      console.warn(
...
export const resolvePackageEntries = (
  ...
  for (const item of raw) {
    ...
    const globalName = resolveManifestName(entry, appConfigDir);
    if (globalName) result.push({ pkg: entry.pkg, globalName });
  }
  return result;
};
